...
| `P0` | 当前 | tools2/model/validation | tools2 Deferred / Trimmed / Obligation 处理闭环 | 建立结构化 boundary inventory 与校验，按 owner occurrence 验证 evidence，保留未解 obligation 并由 check 拒绝；区分全局 inventory 和重复执行 occurrence，不机械复制旧工具遗漏。 | [tools2 修正/完善计划](roadmap/signal-driven-tools2.md) |
...
| `P1` | 进行中 | charter/model/tools2/signal | tools2 启动语义校准 | 独立工具链的 v5 JSON/text 与 animation v3 已闭合；六组启动校准中第 1 组上游构造与 OpenSBI->Kernel 交接、第 2 组 Kernel 与 BootInit 入口均已完成，当前下一批次为第 3 组 `BootInitFlow.Setup`。显式 DSL、handler 命名、continuation 和老工具迁移仍需独立决策，老 tools/ 与静态 trace/SVG 保持默认责任。 | [专题](roadmap/signal-driven-tools2.md) |

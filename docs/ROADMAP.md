...
| `P1` | 待办 | model/semantics | 正式规格化多种上下文和嵌套检查 | 在现有 `exclusive_context` 基础上扩展 context kind 与 effect 体系：资源独占上下文由锁人工建立，系统独占上下文可天然存在或由关闭中断/关闭抢占建立。定义上下文 effect 偏序、嵌套叠加规则和违例检查；从外到内只能加强约束，不能在不可中断/不可抢占/不可睡眠的累计上下文中嵌套会削弱这些约束的上下文。该任务也是主规格中“不同上下文可访问不同层级对象句柄”的具象化。 | [model SEMANTICS](../spec/model/SEMANTICS.md#sem-context-nesting-001-context-effects-compose-monotonically) |
| `P1` | 待办 | model/arceos_ex | CurrentCPU 与 RawSpinLock 建模收敛 | 先把每个 CPU 天然拥有且独立于 `CpuGroup` 的 `CurrentCPU` 正式规格化，并让 `CurrentCPU` 拥有对应 CPU 对象、由 `CpuGroup` 维护这些 CPU 对象的引用；再建立 CPU-local interrupt、current task slot、task preemption 和 `RawSpinLock.LockIrqSave/UnlockIrqRestore` 的驱动链；随后调整 `KernelInitTask.Enable` 的 `within WakeUpNewTaskContext` 进入/退出边界。规格验证通过后必须检查代码生成影响，必要时重新生成受影响对象、trace、KUnit 和 smoke，并运行实现验证。 | [model plan](../spec/model/SEMANTICS.md#sem-current-cpu-plan-001-currentcpu-is-the-per-cpu-self-identity-entry); [coding requirements](../spec/coding/arceos_ex.md#currentcpu-与锁上下文变更后的生成要求) |
...
| arceos_ex | `RestInitPhase` 最小闭环 | 已正式落到 `spec/model/up-multitask/rest-init/` 和 `impl/arceos_ex/src/phases/up_multitask/rest_init.rs`，覆盖 PID 1、kthreadd、SystemState.Scheduling、kthreadd_done、KernelInitDispatchGate 和 BootIdleRuntime。 |

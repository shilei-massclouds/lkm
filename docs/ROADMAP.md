...
| `P1` | 进行中 | arceos_ex/allocator | 建立 KernelHeap/Allocator facade 作为动态容器前置任务 | 当前 OF platform device population 需要 `Vec<PlatformDevice>` 和 `Vec<DeviceRef>` 这类动态容器。`Vec` 依赖实现 `core::alloc::GlobalAlloc` 的全局 allocator；全局 allocator 应依赖 `kmalloc`/SLUB 的公开 alloc/free API；SLUB 再依赖 buddy/PageAllocator 的页分配 API 获取页。当前已完成 PageMetadataMap 与最小 buddy handoff：`PageAllocator.setup()` 会从 MemBlock usable-reserved 构造 per-zone/order intrusive free lists，并由 `page_allocator` KUnit 验证 handoff 页数。下一前置任务是补正式 `alloc_pages/free_pages` API，再让 SLUB/kmalloc 接入，最后推进 `KernelHeap/GlobalAlloc -> Vec` 链路。 | [分配器 Facade](../spec/compose/README.md#分配器-facade) |
| `P1` | 待办 | arceos_ex/smoke | 补齐内存分配 API 运行期 smoke | 当前已覆盖 `MemBlock::alloc_phys()` checkpoint probe、PageAllocator handoff KUnit，以及 page_allocator smoke 的 buddy ready/计数/metadata 只读检查；尚未覆盖正式页分配释放 API。smoke 应按依赖顺序补齐：先测 buddy/PageAllocator `alloc_pages/free_pages` 分配、线性映射读写与释放，再测 SLUB/kmalloc 小对象分配、释放和读写，再测 `GlobalAlloc`/`Vec` grow 路径；`vmalloc/vfree` 可在对应 API 实现后单独补充。测试应保持不为 smoke 暴露无必要内部状态。 | [smoke 测试边界](../spec/coding/arceos_ex.md#smoke-测试边界) |

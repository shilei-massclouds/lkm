...
1. 推进 `ioremap/vmalloc` 泛化：`console/earlycon handoff` 和 handoff 后输出去重已经完成；当前已收紧首个 runtime mapping window 边界和重复映射拒绝，下一步重点是扩展运行期页表安装池以支持更多 vmalloc window。
...
2. 泛化当前运行时映射实现。当前支持的是最小预分配 vmalloc L1/L0 窗口，已显式拒绝跨出首个 runtime mapping window 的映射和同一 busy area 上的重复 mapping record；后续应扩展运行期页表安装池以支持更多 vmalloc window、更多 vmap area、释放/拆映射边界和更完整的 `vm_struct/vmap_area` 元数据。这里要继续保持 Linux-like 分工：`VmallocAllocator` 管 VA 区间和页表映射执行，`Ioremap` 管设备物理资源来源和 MMIO 属性策略。
...
| `P1` | 进行中 | arceos_ex/mm | 泛化 ioremap/vmalloc 运行时映射 | 已先收口规格：`VmallocAllocator` 管 vmap VA 区间和每次 `map_page_range()` 的映射执行/`VmapMapping` 记录，`Ioremap` 管设备物理资源来源与 MMIO 属性策略。实现侧已补齐单个 `VmapArea` 的 `vm_struct/vmap_area` metadata fact、每个 `VmapMapping` 的 record-created fact，并新增 `vmalloc_mapping.multi_record` KUnit 覆盖多次 `map_page_range()` 产生多个独立 mapping record；已补 `UnmapPageRange`/`FreeVmArea` 规格和 `vmalloc_mapping.unmap_free` KUnit 覆盖先 unmap 后 free、重复 unmap/free 拒绝；`Ioremap.iounmap()` 已接到 vmalloc/vmap teardown 路径，并由 `vmalloc_mapping.ioremap_iounmap` 覆盖 cookie retired 与重复 iounmap 拒绝；MMIO 属性首轮已显式区分 plain device/non-cache/write-combine/normal memory alias，当前仅 plain device 支持；已收紧首个 runtime mapping window 边界和重复映射拒绝策略。下一步是扩展运行期页表安装池以支持更多 vmalloc window。 | [arceos_ex 说明](../spec/coding/arceos_ex.md#mmcoreinitphase-编码约束) |

...
| `P1` | 待办 | arceos_ex/allocator | 查明 initcall 阶段动态分配压力失败原因 | 在 OF platform population 实现中，曾尝试把临时 `OfPlatformCandidateSet` 也改为 `Vec`，并为每个 `PlatformDevice` 复制/leak node name；`make test` 在 KUnit/APP 路径触发 `arceos_ex allocation error`。当前已把 candidate set 恢复为固定临时 scratch，并让 `PlatformDevice` 长期保存 `DeviceNodeId` 后通过 `DeviceTree` 解析 name/compatible，正式 `PlatformDeviceStorage` 仍使用 `Vec<Pin<Box<PlatformDevice>>>`。后续需要定位根因：检查 GlobalAlloc layout、kmalloc size class/slab 数量、连续 initcall 分配峰值、Box/Vec 分配尺寸、kfree 回收路径和 smoke/KUnit 对 allocator 的干扰；补针对“多 Vec + 多 Box/pin + initcall 期间批量小对象”的 allocator pressure smoke，再决定扩展 SLUB/kmalloc 还是调整平台对象存储策略。 | [arceos_ex 说明](../spec/coding/arceos_ex.md#mmcoreinitphase-编码约束) |

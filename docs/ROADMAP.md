...
| `P1` | 进行中 | arceos_ex/mm | 泛化 ioremap/vmalloc 运行时映射 | 已先收口规格：`VmallocAllocator` 管 vmap VA 区间和每次 `map_page_range()` 的映射执行/`VmapMapping` 记录，`Ioremap` 管设备物理资源来源与 MMIO 属性策略。实现侧已补齐单个 `VmapArea` 的 `vm_struct/vmap_area` metadata fact、每个 `VmapMapping` 的 record-created fact，并新增 `vmalloc_mapping.multi_record` KUnit 覆盖多次 `map_page_range()` 产生多个独立 mapping record；已补 `UnmapPageRange`/`FreeVmArea` 规格和 `vmalloc_mapping.unmap_free` KUnit 覆盖先 unmap 后 free、重复 unmap/free 拒绝；`Ioremap.iounmap()` 已接到 vmalloc/vmap teardown 路径，并由 `vmalloc_mapping.ioremap_iounmap` 覆盖 cookie retired 与重复 iounmap 拒绝；MMIO 属性首轮已显式区分 plain device/non-cache/write-combine/normal memory alias，当前仅 plain device 支持。下一步继续扩展跨 vmalloc 窗口映射和重复映射策略。 | [arceos_ex 说明](../spec/coding/arceos_ex.md#mmcoreinitphase-编码约束) |

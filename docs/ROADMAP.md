...
| `P1` | 进行中 | arceos_ex/allocator | 建立 KernelHeap/Allocator facade 作为动态容器前置任务 | 当前 OF platform device population 需要 `Vec<PlatformDevice>` 和 `Vec<DeviceRef>` 这类动态容器。`Vec` 依赖实现 `core::alloc::GlobalAlloc` 的全局 allocator；全局 allocator 应依赖 `kmalloc`/SLUB 的公开 alloc/free API；SLUB 再依赖 buddy/PageAllocator 的页分配 API 获取页。当前 `PageAllocator`、`kmalloc/SLUB` 只发布 Ready 事实与元数据，尚无公开 alloc/free API，因此必须先在 model/coding 中补全 `PageAllocator buddy API -> SLUB/kmalloc API -> KernelHeap/GlobalAlloc -> Vec` 链路，再推进 platform bus 填充实现。 | [分配器 Facade](../spec/compose/README.md#分配器-facade) |
| `P1` | 待办 | arceos_ex/smoke | 补齐内存分配 API 运行期 smoke | 当前可执行覆盖限于 `MemBlock::alloc_phys()` 的 checkpoint probe；`PageAllocator`、`kmalloc`/SLUB 和 `vmalloc` 目前只发布 Ready 状态与元数据事实，尚无公开 alloc/free API。smoke 应按依赖顺序补齐：先测 buddy/PageAllocator 页分配与释放 API，再测 SLUB/kmalloc 小对象分配、释放和读写，再测 `GlobalAlloc`/`Vec` grow 路径；`vmalloc/vfree` 可在对应 API 实现后单独补充。测试应保持不为 smoke 暴露无必要内部状态。 | [smoke 测试边界](../spec/coding/arceos_ex.md#smoke-测试边界) |

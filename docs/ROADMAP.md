...
1. 推进 `ioremap/vmalloc` 泛化：`console/earlycon handoff` 和 handoff 后输出去重已经完成；当前 runtime page-table metadata 已能覆盖完整 `VMALLOC_START..VMALLOC_END`，并支持按需追加 L1/L0/PTE 页表页，下一步重点是动态 `VmapArea` / `VmapMapping` 记录容量和更完整的 `vm_struct/vmap_area` 元数据。
...
2. 泛化当前运行时映射实现。当前支持的是完整 `VMALLOC_START..VMALLOC_END` runtime vmalloc 范围，已支持跨 L0/PTE window 映射，并能通过 `PageTableCaches`/`PageAllocator` 按需追加 L1/L0/PTE 页表页和稀疏 slot metadata；后续应把当前固定 `VmapArea` / `VmapMapping` 记录存储扩展为动态容量，覆盖更多 vmap area、释放/拆映射边界和更完整的 `vm_struct/vmap_area` 元数据。这里要继续保持 Linux-like 分工：`VmallocAllocator` 管 VA 区间和页表映射执行，`Ioremap` 管设备物理资源来源和 MMIO 属性策略。
...
3. **进行中：继续 ioremap/vmalloc 泛化**：规格边界已收口为 `VmallocAllocator` 负责 vmap VA 区间管理和每次 `map_page_range()` 的映射执行/`VmapMapping` 记录，`Ioremap` 负责设备物理资源来源和 MMIO 属性策略；实现侧已补齐单个 `VmapArea` 的 `vm_struct/vmap_area` metadata fact、每个 `VmapMapping` 的 record-created fact，并新增 `vmalloc_mapping.multi_record` KUnit 覆盖多次 `map_page_range()` 产生多个独立 mapping record；已补 `UnmapPageRange`/`FreeVmArea` 规格和 `vmalloc_mapping.unmap_free` KUnit 覆盖先 unmap 后 free、重复 unmap/free 拒绝；`Ioremap.iounmap()` 已接到这条 vmalloc/vmap teardown 路径，新增 KUnit 覆盖 cookie retired 与重复 iounmap 拒绝；当前已从首个 runtime mapping window 扩展到完整 `VMALLOC_START..VMALLOC_END` 页表 metadata 覆盖，并支持按需追加 L1/L0/PTE 页表页和稀疏 slot metadata；完整范围内跨 window mapping 支持，越过 `VMALLOC_END` 的范围和同一 busy area 上重复 mapping record 都会被拒绝。
...
| `P1` | 进行中 | arceos_ex/mm | 泛化 ioremap/vmalloc 运行时映射 | 已先收口规格：`VmallocAllocator` 管 vmap VA 区间和每次 `map_page_range()` 的映射执行/`VmapMapping` 记录，`Ioremap` 管设备物理资源来源与 MMIO 属性策略。实现侧已补齐单个 `VmapArea` 的 `vm_struct/vmap_area` metadata fact、每个 `VmapMapping` 的 record-created fact，并新增 `vmalloc_mapping.multi_record` KUnit 覆盖多次 `map_page_range()` 产生多个独立 mapping record；已补 `UnmapPageRange`/`FreeVmArea` 规格和 `vmalloc_mapping.unmap_free` KUnit 覆盖先 unmap 后 free、重复 unmap/free 拒绝；`Ioremap.iounmap()` 已接到 vmalloc/vmap teardown 路径，并由 `vmalloc_mapping.ioremap_iounmap` 覆盖 cookie retired 与重复 iounmap 拒绝；MMIO 属性首轮已显式区分 plain device/non-cache/write-combine/normal memory alias，当前仅 plain device 支持；runtime mapping 已扩展到完整 `VMALLOC_START..VMALLOC_END` 页表 metadata 覆盖，并能按需追加 L1/L0/PTE 页表页和稀疏 slot metadata，支持完整范围内跨 window mapping，仍拒绝越过 `VMALLOC_END` 的范围和重复 area mapping。下一步是动态 `VmapArea` / `VmapMapping` 记录容量和更完整的 `vm_struct/vmap_area` 元数据。 | [arceos_ex 说明](../spec/coding/arceos_ex.md#mmcoreinitphase-编码约束) |

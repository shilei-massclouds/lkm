...
| `P1` | 进行中 | arceos_ex/allocator | 建立 KernelHeap/Allocator facade 作为动态容器前置任务 | 当前 OF platform device population 需要 `Vec<PlatformDevice>` 和 `Vec<DeviceRef>` 这类动态容器。`Vec` 依赖实现 `core::alloc::GlobalAlloc` 的全局 allocator；全局 allocator 应依赖 `kmalloc`/SLUB 的公开 alloc/free API；SLUB 再依赖 buddy/PageAllocator 的页分配 API 获取页。当前已完成 PageMetadataMap、最小 buddy handoff 和正式 PageAllocator API：`PageAllocator.setup()` 会从 MemBlock usable-reserved 构造 per-zone/order intrusive free lists，`alloc_pages/alloc_page/free_pages` 可分配、拆分、释放并合并 buddy 页。下一前置任务是让 SLUB/kmalloc 接入这些页分配 API，再推进 `KernelHeap/GlobalAlloc -> Vec` 链路。 | [分配器 Facade](../spec/compose/README.md#分配器-facade) |
| `P1` | 进行中 | arceos_ex/smoke | 补齐内存分配 API 运行期 smoke | 当前已覆盖 `MemBlock::alloc_phys()` checkpoint probe、PageAllocator handoff KUnit、page_allocator smoke 的 buddy ready/计数/metadata 只读检查，以及正式 PageAllocator `alloc_pages/free_pages` 与 `alloc_page/free_pages` 的 order-0 分配、线性映射读写和释放。后续按依赖顺序补 SLUB/kmalloc 小对象分配、释放和读写，再测 `GlobalAlloc`/`Vec` grow 路径；`vmalloc/vfree` 可在对应 API 实现后单独补充。测试应保持不为 smoke 暴露无必要内部状态。 | [smoke 测试边界](../spec/coding/arceos_ex.md#smoke-测试边界) |
| `P1` | 阻塞 | arceos_ex/platform | 实现 OF platform device population | 目标是在 `of_platform_default_populate_init()` 中把从 DeviceTree 遍历出的 OF platform candidates 构造为 `PlatformDevice`，其内嵌 `Device` 通过稳定 `DeviceNodeId` 关联对应 node，注册 core device 后生成 `DeviceRef` 并加入 `PlatformBusSubsysPrivate.klist_devices` 的 `DeviceRefSet`/Vec backing。计划步骤：1. 补 coding 规格说明 `PlatformBus.platform_devices: Vec<PlatformDevice>` 是临时 lifetime owner、`klist_devices` 当前用 `Vec<DeviceRef>` backing；2. 实现 `DeviceNodeId`，提供 `DeviceNodeRef::id()` 和 `DeviceTree::node(id)`；3. 新增 core `Device`、`PlatformDevice`、`PlatformDeviceRef`/`DeviceRef` 最小语义，包括 `set_node`、`register`、`create_from_node`、`to_platform_device_ref`；4. 改造 `PlatformBus` 存储为 `platform_devices: Vec<PlatformDevice>` 和 `klist_devices: Vec<DeviceRef>`，保留现有 smoke driver path；5. 在 `of_platform_default_populate_init()` 中按 candidate 创建 platform device、注册内嵌 device 并 add device；6. 更新 KUnit/smoke，检查 candidate count、platform device count、bus device count 和至少一个 ref->platform device->node compatible 解析；7. 运行 `make test`、`make verify REPORT=graph`、`git diff --check` 并提交。当前阻塞于 KernelHeap/Allocator facade 尚未实现。 | [arceos_ex 说明](../spec/coding/arceos_ex.md#initcallphase-编码约束) |

...
1. 实现 PLIC 驱动并用 UART 驱动验证外部中断链：下一优先级转为 irqchip/irqdomain、PLIC 独立驱动对象、设备树驱动的 PLIC probe/setup、platform IRQ resource 解析、serial8250 IRQ 绑定、handler registry、中断上下文约束和最小外部中断 dispatch；在该链路验证前，serial8250 console 输出继续保持 polling。
2. 维持 `ioremap/vmalloc` 当前首轮成果：runtime page-table metadata 已覆盖完整 `VMALLOC_START..VMALLOC_END`，支持按需追加 L1/L0/PTE 页表页，`VmapArea` / `VmapMapping` 记录已改为动态容器 backing；更完整的 `vm_struct/vmap_area` 行为，例如空洞复用、增强树查找、lazy purge 和并发边界，降为后续补强，除非阻塞 IRQ 路径。
3. 收口 trace/SVG 输出体验并建立快速 CI，作为审阅和回归保护项跟进。
...
## 下一步计划：PLIC 驱动与 UART 外部中断链

下一阶段优先实现 PLIC 驱动，并与 UART 驱动结合验证外部中断链已经建立。console 输出暂不急于切到 interrupt-driven；在 PLIC claim/complete、handler dispatch 和 UART IRQ 触发路径可验证前，serial8250 console 继续保持 polling。当前计划按以下顺序逐项推进：
...
6. **下一步：IRQ domain / source mapping**。建立 PLIC source 到 logical IRQ 的最小 mapping；解析 UART node 的 IRQ resource；`Ns16550aPlatformDriver` probe 记录 UART IRQ，但 console 输出仍保持 polling。
7. **实现：handler registry 与上下文约束**。增加最小 `request_irq` / handler registration 记录；覆盖 duplicate registration 拒绝、未映射 IRQ 拒绝、未启用 PLIC route 拒绝；建立 hardirq context guard，handler 内不能走睡眠或普通进程上下文路径。
...
1. **已完成：收口 console 测试边界**：app smoke 已移除直接操作 console registry / backend 的场景，只保留公开 `printk` 输出、无重复输出和用户可见 payload 结果；default handoff/idempotent、dummy non-match、non-stdout 和 `keep_bootcon` 已迁到 checkpoint KUnit/action-level handler。当前全量验收：`make test` 中 KUnit 48/48、app smoke 36/36，总计 85/85。
...
5. **下一步：实现 PLIC 驱动并验证 UART 外部中断链**：`Plic` 需要作为独立驱动对象建模；其 probe/setup 对应 Linux `init_IRQ() -> irqchip_init() -> of_irq_init()`，在本项目中落在 `IrqTimeInitPhase`，依赖 `DeviceTree.Ready` 和 PLIC `interrupt-controller` 节点，而不是后续 `PlatformBus` driver probe。规格层应表达 `Plic.Preset` 注册 compatible/init callback 到 irqchip init table，`of_irq_init()` 匹配 compatible 后调用 PLIC init；coding 层应表达该 table 由 LDS 段静态收集，类似 initcall 注册。完成 PLIC irqdomain/source mapping、MMIO/ioremap、context threshold、source enable、claim/complete 后，再用 serial8250 IRQ resource 和最小 UART handler 验证 PLIC claim -> irq dispatch -> UART handler -> PLIC complete 链路；console 输出在此期间仍保持 polling。
...
| --- | --- | --- | --- | --- | --- |
| `P0` | 待办 | arceos_ex/irq | 实现 PLIC 驱动并用 UART 验证外部中断链 | 当前下一优先级。`Plic` 必须独立建模，不能只是 `IrqController` 的布尔事实；PLIC probe/setup 对应 Linux `init_IRQ() -> irqchip_init() -> of_irq_init()`，在本项目中落在 `IrqTimeInitPhase`，依赖 `DeviceTree.Ready` 和 PLIC `interrupt-controller` 节点。规格层表达 compatible/init callback 注册到 irqchip init table，`of_irq_init()` 匹配 compatible 后调用 PLIC init；coding 层表达该 table 由 LDS 段静态收集，类似 initcall 注册。建立 irqchip/irqdomain、PLIC MMIO/ioremap、source enable/priority、hart context threshold、claim/complete、serial8250 IRQ resource 绑定、handler registry 和中断上下文约束；近期验收目标是通过 UART 驱动触发并观察 PLIC claim -> irq dispatch -> UART handler -> PLIC complete 的外部中断链。console 输出可继续保持 polling，不能提前声明 interrupt-driven ready。 | [arceos_ex 说明](../spec/coding/arceos_ex.md#consoleearlycon-handoff-编码约束) |
...
| `P2` | 待办 | arceos_ex/console | serial8250 interrupt-driven console | 在 PLIC 驱动和 UART 外部中断链验证完成后，再考虑把 serial8250 console 输出切到中断驱动；当前 polling 输出仍是唯一 ready 路径。 | [arceos_ex 说明](../spec/coding/arceos_ex.md#consoleearlycon-handoff-编码约束) |

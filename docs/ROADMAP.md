...
3. **进行中：继续 ioremap/vmalloc 泛化**：规格边界已收口为 `VmallocAllocator` 负责 vmap VA 区间管理和每次 `map_page_range()` 的映射执行/`VmapMapping` 记录，`Ioremap` 负责设备物理资源来源和 MMIO 属性策略；实现侧首个切片已补齐单个 `VmapArea` 的 `vm_struct/vmap_area` metadata fact 和每个 `VmapMapping` 的 record-created fact，并由 console handoff KUnit 断言。下一步继续扩展多 mapping record 覆盖、unmap/free 边界和 MMIO 属性模型。
...
| `P1` | 进行中 | arceos_ex/mm | 泛化 ioremap/vmalloc 运行时映射 | 已先收口规格：`VmallocAllocator` 管 vmap VA 区间和每次 `map_page_range()` 的映射执行/`VmapMapping` 记录，`Ioremap` 管设备物理资源来源与 MMIO 属性策略。实现侧首个切片已补齐单个 `VmapArea` 的 `vm_struct/vmap_area` metadata fact 和每个 `VmapMapping` 的 record-created fact，并由 console handoff KUnit 断言；下一步继续扩展多 mapping record 覆盖、跨 vmalloc 窗口映射、unmap/free 边界和重复映射策略。 | [arceos_ex 说明](../spec/coding/arceos_ex.md#mmcoreinitphase-编码约束) |

...
8. **下一步：UART 外部中断链验证**。先建立只读 checkpoint KUnit observer：它只能读取 trace/counter/object facts，不得调用 UART handler、root intc entry、PLIC claim/complete、`request_irq` 或 source-enable API，也不得修改 pending/claimed/enable 状态。随后逐步实现 PLIC source enable、claim/complete 和最小 IRQ dispatch；启用 UART 中断条件后，由 observer 验证真实路径 `UART interrupt -> hart external interrupt -> root intc -> PLIC claim -> irq dispatch -> UART handler -> PLIC complete`。

...
2. **P0：原生 `/sbin/init` / OpenRC 当前计划**。当前诊断命令为 `make run APP=user-boot ROOTFS_OVERLAY=none QEMU_APPEND="earlycon=sbi" PROBE=user-syscall-trace,user-syscall-error FORCE=1`。最新证据显示 OpenRC 在 `setsid(157)` 返回 `EPERM` 后反复触达 syscall `137`；本地 Linux 6.12 RISC-V/asm-generic 明确 `__NR_rt_sigsuspend=133`、`__NR_rt_sigtimedwait=137`，因此本片先修正 `137` 的诊断身份。实测参数形态为 `uthese=0x100c9350`、`uinfo=NULL`、`uts=NULL`、`sigsetsize=8`，当前仍返回 `-ENOSYS`。由于 `uts=NULL` 且无 pending matching signal 在 Linux 会等待，不能把当前形态伪装成成功或零超时 `EAGAIN`；本片只补 `rt_sigtimedwait` unsupported 名称和 best-effort 参数复制诊断，不实现 signal wait、pending queue、timeout、scheduler sleep 或 restart syscall。

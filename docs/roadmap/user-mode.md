...
2. **P0：OpenRC getty/login shell 当前计划**。真正阻断闭环的是 login 交互验收：TTY 输入链路已能消费 host 输入，但一次性 delayed stdin 会把后续命令交给 `Password:`；同时 bare Alpine minirootfs 的 `root:*` 被锁定，`ROOTFS_OVERLAY=none` 不应自然登录。显式 account-only `ROOTFS_FILE_OVERLAY_DIR` 继续提供专用测试用户和 test-only 密码 hash，未设置 overlay 时保持 bare rootfs 账号语义。focused run 已越过 `login:` / `Password:`、认证、单层 nested vfork takeover、fd-local `fchown(55)` / `fchmod(52)`、AF_UNIX stream socket fd 创建、`/var/run/nscd/socket` pathname `ENOENT`、root-only `setgroups(159)`、`setgid(144, gid=100)` 和 `setuid(146, uid=1000)` root-euid drop；登录继续经过 `chdir`、`.hushlogin` missing-path 和 MOTD 输出。登录 shell `execve(221)` 返回 `EFAULT`，用户态打印 `login: can't execute '/bin/sh': Bad address`；本片新增的 `PROBE=user-syscall-error` 诊断已把直接边界定位为 `fail_stage=filename_copy` / `fail_reason=filename_copy`，并保留 filename/argv/envp 指针、child continuation 和 argv/envp 前缀事实。不修复 execve 行为，不扩大 credentials、TTY ownership、inode ownership、network 或完整 exec 语义；下一片应基于 filename user pointer/map/copy 事实继续规格化。该 case 暂保留为 `TEST_OPENRC_LOGIN=1` opt-in 诊断；早期 DGRAM syslog path、`sendto(206)`、成功 `connect(203)`、`getgroups(158)` 和 login shell execve 语义继续后续单独规格化。

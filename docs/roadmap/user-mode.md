...
1. **Payload / exec 对象边界**。`PayloadPhase` 已确认为 `Kernel` 生命周期末尾阶段；`UserBootPayload`、`ElfObject`、`UserAddressSpace`、`UserStack`、`UserTrapFrame`、`SyscallException` / `SyscallTable`、`FilesStruct` 和 `KernelInitTask/UserAppFlow` 的模型、coding 与实现事实已经首轮收口。`SyscallException` 沿用 `ExceptionType`，不建立独立 `SyscallDispatcher`；`KernelInitTask/UserAppFlow` 表示 PID 1 的 `KernelInitTask` 经 exec/user entry 后获得用户态身份，不创建第二个 task。

...
2. **P0：原生 OpenRC `rc.local` 当前计划**。当前诊断命令为 `make run APP=user-boot ROOTFS_OVERLAY=none QEMU_APPEND="earlycon=sbi" PROBE=user-syscall-trace,user-syscall-error FORCE=1`。近期目标收窄为 native Alpine `/sbin/init` / OpenRC 能推进到 local service 阶段，执行一个受控、非交互本地启动脚本 marker，例如 `/etc/local.d/*.start` 或等价 `rc.local` 输出/创建文件；不追求完整 OpenRC、getty/login、真实 VT/多 TTY、controlling tty/job-control、daemon supervisor、pidfile、syslog/socket/network 或完整服务生态。当前已越过早期 child record 回收、`/dev/ttyN` `openat(..., O_RDWR|O_NONBLOCK|O_LARGEFILE)` 的旧 `access_mode+unsupported_flags` 边界、首个 `/dev/tty1` fd 上的 `F_SETFL` 边界，以及后续 `/dev/tty2`、`/dev/tty3` 等 alias open 的单 fd/AlreadyOpen 限制；最新复跑显示 OpenRC 继续打开更多 TTY alias 后耗尽当前固定 fd table，新的第一阻断是 `FileError::TooManyOpenFiles`/`EMFILE`。当前分片只解除 TTY alias 多 fd entry 安装：fd table 3..15 选择首个空闲 fd，各 fd entry flags 独立，后端仍共享同一个 console-like `Tty0`，不扩容 fd table。后续分片只围绕阻断 rc.local marker 的真实证据推进：fd 生命周期/关闭继承、`/dev/null` 最小 char-device、bounded `nanosleep`/wait loop、`/sbin/openrc` / `/sbin/openrc-run` / `/bin/sh` / `/etc/init.d/local` / `/etc/local.d/*.start` exec 链，以及脚本常用文件 syscall。

...
2. **P0：OpenRC getty/login shell 当前计划**。真正阻断闭环的是 login 交互验收：TTY 输入链路已能消费 host 输入，但一次性 delayed stdin 会把后续命令交给 `Password:`；同时 bare Alpine minirootfs 的 `root:*` 被锁定，`ROOTFS_OVERLAY=none` 不应自然登录。当前推进片新增显式 account-only `ROOTFS_FILE_OVERLAY_DIR`，用专用测试用户和 test-only 密码 hash 提供登录凭据，同时保持未设置该 overlay 时的 bare rootfs 账号语义不变。focused run 已越过 `login:` / `Password:` 输入和认证；旧直接阻断是 BusyBox login 认证后在现有 OpenRC/getty child continuation 内调用 `clone(220)` vfork，返回 `ENOSYS` 并打印 `login: vfork: Function not implemented`。当前片已闭合该单层 nested vfork takeover：同一 internal UserChild execution slot 取得递增 user-visible pid 并进入 child continuation，不实现 pidfd nested vfork、多 runnable user task ref 或完整 parent/child 并发。本片也已闭合登录后 fd-local `fchown(55)` / `fchmod(52)`：`PROBE=user-syscall-trace` 复现 `fchown(fd=0, uid=1000, gid=100)` 和 `fchmod(fd=0, mode=0600)` 均返回 0。修后 focused run 仍未到 shell prompt，新的 post-auth 第一 unsupported 边界记录为 `socket(198)`；guest 文本仍显示 `login: can't set groups: Function not implemented`，但该文本不是首个缺失 syscall 的可靠名称，本地 asm-generic 确认 `198=socket`、`159=setgroups`。该 case 暂保留为 `TEST_OPENRC_LOGIN=1` opt-in 诊断；下一片若继续推进，必须先规格化 `socket(198)` 是否只是 syslog/noisy path 还是登录失败直接原因，`sendto(206)`、`setgroups(159)`、完整 credentials、TTY ownership 和 inode 持久化继续 deferred。

...
| 6. payload 预提交与交接 | `PayloadHandoffPreparePhase`、KernelInitFlow Online、`CommitPayloadHandoff`，以及 Hello/Smoke/UserBoot 各自的 no-return 或 replacement 边界 | `KernelInitFlow.Enable` 发送前 | `KernelInitFlow.PayloadHandoffCommitted` 或对应的确定失败边界 |

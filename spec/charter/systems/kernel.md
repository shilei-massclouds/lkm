...
* OnEnable：只接受 OpenSBI 的真实入口交接。必须验证 `Riscv64Platform`、`OpenSBI`、`Riscv64`、
  ...
  KernelInitFlow 交付 contextual Enter。KernelInitFlow 在 Task 发布时已经 Online；初始 context coordinate
  转到 `kernel_init_entry()`，由 `KernelInitFlow.RunKernelInit` 执行 runtime/payload 叶子。该跨栈 continuation 仍承载同一个
...
* Online：应用运行环境已经准备就绪，等待应用启动。提交后 Kernel 作为唯一发送者异步发送
  `KernelInitFlow.Action::CommitPayloadHandoff`。该 action 的 UserBoot ApplicationInstance replacement 或 Hello/Smoke
...
一次。`BootInitFlow` 是 `BootTask.flow` 指向的终身 TaskFlow；Kernel.Enable 必须先完成自身
...
`yields Schedule()`；Kernel.Enable 不直接驱动 Scheduler 内部步骤，`yields` 也不隐式产生 context 或
Task lifecycle 效果。
...
KernelInitFlow.Enter 消费当前 context coordinate，初始坐标转到 `kernel_init_entry()` 并执行
`KernelInitFlow.RunKernelInit`；不回调 Kernel Setup/Enable，也不发送 Startup。

KernelInitFlow 的 `RunKernelInit` 正文 Action 直接驱动 `PreSmpInitPhase`、`SmpBringupPhase`、
...
ApplicationInstance，Hello/Smoke 进入同一 KernelInitFlow 下的内核态 no-return entry。

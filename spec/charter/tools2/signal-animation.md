...
动画采用独立 `animate` 阶段，且只消费 tools2 version 10 `model.json` 与 `view.json`。model 提供
...
- `yields` 创建 token 时生成 `<source-signal-id>:yield:<token-id>`，目标完成生成 settle；匹配
  `yield_token_resumed` 生成 `<source-signal-id>:resume:<token-id>`。yield/resume 保留 token、lane、resume
  coordinate、resume Signal 与 identity-return 事实；token consumed 是精确一次协议证据，不再生成重复
  视觉时刻。
...
- moment kind 只取 `request`、`feedback`、`settle`、`yield`、`resume` 或 `terminal`；最终 `completed`、`rejected`、
  `failed`、`truncated`、`stopped` 继续由独立 outcome 字段记录。request 携带 source→target transfer，
  feedback 携带 target→source transfer，settle/yield/resume/terminal 不携带 transfer。
- 所有 moment 严格按 v10 view event `sequence` 排列。同步 `drives` 子 Signal 的 request/feedback 位于父
...
animate 输出内嵌 `lkm.spec.signal-animation` version `4` 的自包含 HTML。animation v4 是由 v10 输入
确定生成的播放协议：`moments` 与 `frames` 一一对应；moment ID 固定为 `<signal-id>:request`、
`<signal-id>:feedback`、`<signal-id>:settle`、`<signal-id>:yield:<token-id>`、
`<signal-id>:resume:<token-id>` 或 `<signal-id>:terminal`，kind 与 ID 中的控制阶段一致。
...
包含关系判定，不进入 animation v4 或 Python frame。
...
tools2 v10 model/view 的结构化字段提供上述生成语义；animation 封装协议为 version 4。新生成器和
新 bundle 只发布 v4，不提供生成 v1/v2/v3 的兼容开关；已经生成的自包含
v1/v2/v3 HTML 仍可使用其各自内嵌播放器独立打开。前端不得从显示名称推断 handler/state，也不得复用老 `tools/`
静态 SVG renderer。

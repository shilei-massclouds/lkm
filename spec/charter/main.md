...
1. [系统分析与建模流程](system-analysis-and-modeling-process.md)：分析与建立各级系统模型的通用流程。
...
本文是目标内核项目的项目章程，是源头性的自然语言说明文档。它整理人类对本项目的构想、目标、指导思想、核心概念、需求分析和演化规划，以符合人类直觉、便于人类理解和决策的方式说明本项目希望内核应该是什么样子。Charter 是项目设计意图的最高语义权威。

本文不是形式化规格本身。文中可以保留概念定义、设计背景、路径规划、对象解释和参考边界；其中需要被工具检查、推导、生成或实现约束化的内容，应逐步沉淀到对应下层规格。形式化程度不改变语义权威：下层可以消除歧义和增加细节，但不能借助更精确的表达改变 Charter 的含义。

## 核心语义细化链

本项目只有一条核心语义细化链：`Charter -> Model -> Coding -> Impl`。四层职责如下：

1. `Charter` 用人类可理解的自然语言确定设计意图、目标、概念和长期边界，是最高语义权威。
2. `Model` 用偏形式化语言精确化 Charter，把适用的对象、状态、事件、依赖和证明边界变成可检查条目；它只能形式化和细化 Charter，不得改变其含义。
3. `Coding` 约束 Model 到代码的映射，可以具体到数据结构、算法、函数与模块内落点、内存布局、寄存器和架构/语言安全边界；它不得重新定义 Charter 或 Model 已经确定的语义。
4. `Impl` 实现 Charter、Model 和 Coding 共同要求的效果。实现可以承载和证明规格，但不具有依据现状反向解释或改写规格的权力。

Coding 中的“映射”只指把 Model 元素约束为任一符合规格的 Impl 所使用的代码表示。它不负责建立或维护
两个具体实现之间的源码、符号、函数或 checkpoint 对应关系，例如 `arceos_ex` 与 Linux 或 ArceOS 之间的对照。
参考实现可以作为 Charter/Model 决策、Coding 表示选择或验证的依据；要进入 Coding 核心规格，其结论必须改写为
Model 到代码表示的约束。两个具体实现之间的静态对照、差分观测和 checkpoint mapping 属于独立的 cross-reference 或验证支撑责任，
不是 Coding 的核心语义责任。

每个下层都必须同时满足所有适用上层约束，而不是只满足紧邻的一层。只有 Charter、Model 和 Coding 中所有适用上层都未约束的细节，较低层才可以局部自由选择；这种自由选择本身不自动成为规格。若选择形成新的可观察行为、接口或对象边界，就必须先提升到适当的核心规格层明确，并再次沿核心链向下闭合。

层间冲突按 `Charter -> Model -> Coding -> Impl` 的顺序解决：修正不符合约束的较低层，不得用现有实现、测试、roadmap 或装配现状反向覆盖较高层。若 Charter 的含义不足以做出决定，或目标与 Charter 冲突，应停止向下细化并回到 Charter 决策；这项语义顺序不构成编辑已锁定 Charter 的授权。

`guidance`、`compose` 和 `testing` 是支撑核心链的旁支职责。Guidance 约束变更和生成工作流；Compose 在适用时约束 Coding 到 Impl 的装配、封装和发布；Testing 验证核心规格与实现的一致性。它们都不得反向定义核心语义；Compose 与 Coding 冲突时以 Coding 为准，测试与核心规格冲突时应修正测试或实现，而不是修改高层规格来迎合现状。
...
- `Kernel.Enable` 同步驱动 BootInitFlow 三段 lifecycle；BootInitFlow Online 后，同一固定 Flow 对
  ...
  Suspend prev、Dispatch next，再向 next 固定 TaskFlow 交付 contextual Enter；Kernel 不是 Schedule
...
- KernelInitFlow 随 KernelInitTask 发布为 Online；首次及恢复派发都由 contextual Enter 承载。
  初始 Enter 消费 `RunKernelInit` 正文坐标，直接驱动 PreSMP/SMP bringup、runtime、initcall、rootfs、finalize、PayloadPrepare 和
...
### SmpbootThread 模板与 Task lowering
...
模板创建的 Task/Flow 按统一生命周期推进；下表中的 `SmpbootThread.*` 只表示创建策略投影：

| Lifecycle process | state_effect | property_effect | 语义 |
| --- | --- | --- | --- |
| `SmpbootThread.preset()` | `always` | 分配 fresh TaskRef，绑定 CPU、模板和实例名称，并声明初始 Flow | 模板已注册，所属 CPU 存在；Task/Flow identity 此时独立建立。 |
| `SmpbootThread.setup()` | `always` | setup 统一 Task、写入 per-CPU store，并 park | 对应 `CPUHP_CREATE_THREADS` / `smpboot_create_threads(cpu)` / `__smpboot_create_thread()`；Task 与初始 Flow 均可查询，角色视图为 `Parked`。 |
| `SmpbootThread.enable()` | `conditional` | enable Task/Flow 并 unpark | 对应 `CPUHP_AP_SMPBOOT_THREADS` / `smpboot_unpark_threads(cpu)`；实际可调度 identity 是 TaskRef。 |
| `SmpbootThread.disable()` | `conditional` | 先 disable/cleanup Flow，再 park 或停止 Task | 当前启动主线不展开；CPU offline / teardown 路径再细化。 |
...
阶段对象不是普通数据对象的简单容器，也不只是一个顺序控制器。它至少承载三类信息：当前阶段的初始化逻辑、当前阶段的默认上下文，以及当前阶段与父子阶段之间的边界。阶段之间不因目录位置或同级关系自动首尾相接；父 transition 通过 `drives` 选择并排序子阶段，阶段自身通过 `emits` 推进 Preset、Setup、Enable。每个阶段对象都可以定义自己的默认上下文，用于描述该阶段中普通对象建立和资源访问所处的背景条件。
...
#### 第三阶段：单核多任务期（BootInitFlow Enable）
...
8. `地址转换控制面`，英文名 `Vm`：协调 CPU 在 `PhysicalDirect`、`TrampolineVm`、`EarlyVm` 与
   `SwapperVm` 之间切换；它不是内核地址空间本体。唯一内核地址空间资源由独立的
   `KernelAddrSpace` 系统表示。
...
在阶段对象中，`BootInitFlow` 是内核引导过程中最早出现的启动执行阶段。静态 `BootTask` 是其 parent，并从首个入口 checkpoint 起已在 boot CPU 上处于 OnCpu；`BootTask.stack` 是该 Task 的值属性，不是子对象。BootInitFlow.Setup 直接编排区间还会为 `BootTask` 补充 `InitMM` 子对象，用于描述 Linux `init_mm` 这类根任务关联的地址空间元数据。每个 CPU 拥有自己的 `TrapType`；`TrapType` 再拥有 `InterruptType` 与 `ExceptionType`，维护二者共用的总入口和分流前公共边界。

地址空间资源与地址转换控制面是两个独立边界。`KernelAddrSpace` 拥有 `KernelImage`、`FixMap`、
`LinearMap` 与 `UserSpaceReserve`；`Vm` 拥有四个共享 controller。入口前导期只准备并使用
`TrampolineVm` 与 `EarlyVm`，`SwapperVm` 留给后续阶段。各系统的自然语言语义由
[`objects/`](objects/README.md) 下对应独立文件定义。
...
4. `中断资源`（`InterruptType`）
   ...
   * `setup` - 关闭中断总门控
     ...
     * 执行动作：关闭 BootCPU 的中断总门控。这里的动作仍属于防御式过程，即使固件交接时已经关闭总门控，内核入口仍再次规范化其状态。
     * 结束状态：`InterruptType` 进入 `Ready`，确保中断进入路径在分路门控和总门控两个层面都被封闭。
...
8. `内核地址空间`（`KernelAddrSpace`）与控制面（`VM`）
   ...
   `KernelAddrSpace` 是唯一内核地址空间资源；`VM` 是独立的地址转换控制面。入口前导期由
   `Vm.Preset` 准备早期布局和 controller，但不切换当前 CPU；随后 `Vm.Setup` 依次完成
   `PhysicalDirect -> TrampolineVm -> EarlyVm`，使启动 CPU 进入早期虚拟地址环境并提交 `Vm.Ready`。
   `Vm.Enable` 保留给后续建立并启用 `SwapperVm`，本段不触发。

   本组中的每个系统均由独立 Charter 文件定义：[`KernelAddrSpace`](objects/kernel-address-space.md)、
   [`FixMap`](objects/fix-map.md)、[`LinearMap`](objects/linear-map.md)、
   [`UserSpaceReserve`](objects/user-space-reserve.md)、[`RawDtb`](objects/raw-dtb.md)、
   [`Vm`](objects/vm.md)、[`PhysicalDirect`](objects/physical-direct.md)、
   [`TrampolineVm`](objects/trampoline-vm.md)、[`EarlyVm`](objects/early-vm.md) 与
   [`SwapperVm`](objects/swapper-vm.md)。本节只保留入口索引和编排摘要，不代替各系统的权威定义。

9. `处理器管理`（`CPUGroup`）

   描述：`Kernel` 的直接子对象，与 `片上系统` 平级；通过 `cpus[logical_id]: CPU` indexed-owned collection 唯一拥有所有 CPU 实例。`BootCPU := CpuGroup.cpus[0]`，其它角色别名同样不得建立独立状态或身份。possible/present/active/online 集合从 CPU 元素状态派生。
...
本小节补充 `BootInitFlow.Preset` 如何编排入口对象的建立顺序。Preset 在推进过程中直接调用各对象过程；全部对象事实成立后提交 `BootInitFlow.Prepared`，形成 `BootInitFlow.Setup` 的起点边界。这里没有额外的入口 PhaseObject 或 cleanup lifecycle。
...
13. 执行 `片上系统.preset()`，完成片上系统平台相关的早期预置，并直接提交 `BootInitFlow.Prepared`；
    completion 随后直接进入 `start_kernel` 所代表的 `BootInitFlow.Setup` 起点，不增加 Action、
    continuation、Kernel drive 或 lifecycle。
...
- 准备期输入对象模型
- `Kernel` / `BootInitFlow` 启动树，其中 Preset 直接编排入口对象，Setup 直接编排到
  `setup_arch()` 返回，再驱动 `CorePreparePhase` 和 `MmCoreInitPhase` 等直接子阶段对象模型
...
#### BootInitFlow.Setup 直接编排：从 start_kernel() 到 setup_arch() 返回

这不是独立子阶段或 PhaseObject。`BootInitFlow.Setup` 从 `start_kernel()` 入口直接驱动对象，经过
`paging_init()` 建立完整内核虚拟地址空间，并继续执行 `setup_arch()` 后半段，直到函数返回。

从 Linux 代码边界看，起点是 `init/main.c` 中进入 `start_kernel()`，主体发生在
`arch/riscv/kernel/setup.c` 的 `setup_arch()` 内，结束边界是 `setup_arch()` 返回。`paging_init()`
内部调用 `setup_vm_final()`，建立 `swapper_pg_dir` 中的 fixmap、线性映射和内核映像映射，并将
`RISCV64.satp` 切换到完整内核页表；其后的 DeviceTree、zone/page metadata、resource、CPU topology、
CBO/hwcap 与 DMA/cache policy 仍属于同一 BootInitFlow 直接编排区间。

因此，本区间先推进 `SwapperVM` 和 `VM` 到完整内核地址空间，再完成 `setup_arch()` 返回所需的
体系结构对象事实。该边界不表示 page allocator、NUMA、vmalloc、用户地址空间或后续运行期内存管理
已经完成。

本区间仍处于 `System Exclusive` 上下文；中断、其它任务和其它 hart 的并发进入路径均未开放。

当前先识别出本直接编排区间的核心对象和边界：

1. `BootInitFlow.Setup` 直接编排：它从 `BootInitFlow.Prepared` / `start_kernel()` 起点接续入口前导
   步骤，不创建 wrapper instance、parent、状态或 lifecycle；`setup_arch()` 返回也不新增 checkpoint。
...
5. `早期设备树对象`（`EarlyDtb`）：入口前导期已按规格前置证明验证并映射原始 `RawDtb`；BootInitFlow.Setup 直接编排区间短暂建立 `EarlyDtb`。它通过 `preset` 和 `setup` 分担职责：`EarlyDtb.preset()` 先从原始 `dtb` 中提取最小平台事实，解析 `/cpus` 建立 `PlatformCpuInfo` 并检查启动 hartid 合法，解析 `/memory` 建立 `PhysicalMemory`；`EarlyDtb.setup()` 再完成本区间对 DTB 的后续使用，提取 kernel command line，解析 FDT header `/memreserve/` 与 `/reserved-memory` 得到固件和平台保留内存范围，并触发 `MemBlock.preset()` 建立候选物理内存区段。本区间末尾 `EarlyDtb` 退出服务。后续正式 OF/DeviceTree 对象若建立，应视为基于原始 `dtb` 或早期解析结果重新建立的运行期对象，不是 `EarlyDtb` 的简单延续。
6. `命令行管理对象`（`CommandLine`）：代表启动命令行的文本视图管理对象。BootInitFlow.Setup 直接驱动 `CommandLine.preset()` 建立 raw view，即 `KernelCmdline`；完成 `parse_dtb()` 后，`CommandLine.state == Prepared`、`KernelCmdline.state == Ready`。`SavedCommandLine` 与 `StaticCommandLine` 由后续 CorePreparePhase 建立。
7. `参数解析管理对象`（`Params` / `EarlyParam` / `BootParam` / `PayloadParam`）：`Params` 是参数解析类对象的顶层管理者。BootInitFlow.Setup 由 `Params.preset()` 驱动 `EarlyParam.setup()`，完成第一次 `parse_early_param()`；CorePreparePhase 再由 `Params.setup()` 驱动 `BootParam.setup()` 和 `PayloadParam.setup()`，并保留 `print_unknown_bootoptions()` checkpoint。
8. `启动根任务栈保护动作`：BootInitFlow.Setup 的第一项调用 `BootTask.EnableStackGuard`；通用 Action 的栈归属、有效范围、幂等性、失败边界和完整性检查语义由 [`Task` owner](objects/task.md#栈底保护-action) 定义，本编排只规定 BootTask 实例上的调用顺序。
...
12. `静态分支对象`（`StaticBranch`）：`setup_arch()` 中的早期 `jump_label_init()` 调用位置保留，
    但当前对象推进绑定到 `setup_arch()` 返回后的通用 `start_kernel(): jump_label_init()`，属于
    `CorePreparePhase`。`StaticBranch.setup()` 建立静态分支表、排序分支项，并建立 static key 到分支点
    的关联；后续 `jump_label_init_ro()` 对应 `StaticBranch.enable()`。当前
    `CONFIG_JUMP_LABEL=y`、`CONFIG_STRICT_KERNEL_RWX=y`，禁用配置不进入当前边界。
...
对 `BootCPU` 而言，Linux 在 `smp_setup_processor_id()` 中先把入口前导期保存的值写为 BootCPU 的物理 `hartid`，随后 `boot_cpu_init()` 把启动 CPU 同时标记到 `possible`、`present`、`active` 与 `online` 四类 CPU 位图中。规格层不照搬位图实现，但必须按这两个顶层函数保留 `PlatformCpuInfo`/BootCPU 的先后系统边界。

BootInitFlow.Setup 直接编排对象；编排 owner 是 `BootInitFlow`，不是独立阶段对象。
`EarlyDtb` 基于入口前导期已经验证过的 `RawDtb` 建立
`PlatformCpuInfo` 与 `PhysicalMemory`，并驱动 CommandLine/MemBlock；Params 驱动 EarlyParam/EarlyCon；
SwapperVM 替换 EarlyVM，EarlyIoremap 与 SBI 建立服务事实。BootTask、InterruptType 与 CPUGroup 是
入口前导期承接对象，CPU-local 步骤继续通过 `CurrentCPU` 解析到 `CpuGroup.cpus[0]`。
...
Kernel 通过 BootTask/BootInitFlow 串接入口对象、Setup 直接对象编排、CorePreparePhase 与
MmCoreInitPhase 等内部规格。内存核心初始化的正式模型位于
...
- `DeviceTree`、`Zones`、`PageMetadataMap`、`ResourceTree`、`CpuGroup`、`CacheBlockInfo`、
  `CpuCapabilities` 与 `DmaCachePolicy` 均为 Ready，证明 `setup_arch()` 后半段已经完成

当前 BootInitFlow.Setup 直接对象模型已经覆盖 `BootTask.stack` guard、`InitMM`、`InterruptType`、
BootCPU、`EarlyIoremap`、`SBI`、CommandLine raw view、EarlyParam/EarlyCon、PrintkBuffer、MemBlock、
EarlyDtb、SwapperVM/VM，以及 `setup_arch()` 后半段的正式平台对象。`efi_init()` 仍为 deferred。

##### BootInitFlow.Setup 顶层函数与系统步骤

本小节按 Linux 顶层函数粒度列举 BootInitFlow 直接编排步骤；每个顶层函数都必须映射到一个明确的
...
当前直接对象模型已经落地；完成 `setup_arch()` 后才到达核心准备期起点。默认
`charter-first` 下，后续围绕 `setup_arch()` 返回到 `trap_init()` 的讨论应先落入下面的核心
准备期规格，再视稳定程度同步到形式化模型和对象级实现；显式 `model-first` 轮遵守
[`spec/guidance`](../guidance/README.md) 的确认门禁。

#### 子阶段 3：核心准备期（Core Prepare Subphase）

本子阶段覆盖从 `setup_arch()` 返回到 `trap_init()` 完成。它接受 BootInitFlow 直接编排已经建立的
完整内核虚拟地址空间、平台描述、CPU 能力与 DMA/cache policy 事实，并继续完成通用运行期基础设施
和正式异常分发框架的准备。

从 Linux/RISC-V 代码边界看，本阶段只对应 `init/main.c` 中 `setup_arch()` 返回后到 `trap_init()`
完成之间的通用初始化。
...
1. `核心准备期对象`（`CorePreparePhase`）：从 `setup_arch()` 返回边界开始，负责编排返回后的通用
   初始化，并以 `trap_init()` 完成作为阶段边界。
...
10. `命令行管理对象`（`CommandLine`）：`KernelCmdline` raw view 由 BootInitFlow.Setup 建立；`SavedCommandLine` 与 `StaticCommandLine` 由 CorePreparePhase 的 `setup_command_line()` 对应步骤建立。
...
13. `参数解析管理对象`（`Params`）：`Params` 在 BootInitFlow.Setup 已由 `preset` 推进到 Prepared；核心准备期由 `Params.setup()` 驱动 BootParam/PayloadParam。
14. `普通启动参数对象`（`BootParam`）：对应 `parse_args("Booting kernel", ...)`。它不同于 BootInitFlow.Setup 中的 EarlyParam。
...
16. `随机性对象`（`Randomness`）：本阶段只执行 `Randomness.preset()`，对应 `random_init_early(command_line)`，表示早期架构/非 timekeeping 熵源已经混入；完整 RNG Ready 留给后续 `Randomness.setup()`。
17. `打印缓冲区`（`PrintkBuffer`）：BootInitFlow.Setup 已以 `PrintkBuffer.preset()` 建立静态缓冲；本阶段增加 `PrintkBuffer.setup()`。
18. `异常表`（`ExceptionTable`）：对应 `sort_main_extable()`。`ExceptionTable.setup()` 明确要求主异常表已经排序且查询机制可用，以便指导实现阶段执行排序而不是只记录“已调用”。
19. `异常资源`（`ExceptionType`）及其子异常对象：对应 `trap_init()`，建立正式异常分类、分发和 handler 注册框架。`PageFaultException`、`BreakpointException`、`UnexpectedException` 在本阶段进入 `Ready`；`SyscallException` 保持 `Prepared`，等待后续用户态执行路径。
...
图 12 用于说明核心准备期涉及对象的分类与相互关系。`CorePreparePhase` 接受 BootInitFlow.Setup
已完成 `setup_arch()` 的前置事实，只推进返回后的 per-cpu、命令行与启动参数、随机性、日志、异常表
和异常流。图中的 deferred/impl checkpoint 区域表示保留调用位置但暂不进入 formal object 的路径。

本子阶段的结束状态至少包含：

- `BootInitFlow.state == Prepared`，且 `setup_arch()` 直接对象事实已经完成
- `VM.state == Online`
...
- `ExceptionType.state == Ready`，表示正式异常分类和分发框架已经建立
...
图 13 只表示 `setup_arch()` 返回后到 `trap_init()` 的通用核心准备。`setup_arch()` 后半段已经由
BootInitFlow.Setup 直接编排，不再属于 CorePreparePhase。

##### 核心准备期过程处理清单

##### BootInitFlow.Setup 的 `setup_arch()` 后半段前置事实（不属于 CorePreparePhase）
...
| `misc_mem_init(): sparse_init()` | trimmed | 当前 `CONFIG_FLATMEM=y`、`CONFIG_SPARSEMEM=n`，该调用展开为空操作。 |
| `misc_mem_init(): local_flush_tlb_kernel_range(VMEMMAP_START, VMEMMAP_END)` | deferred | SPARSEMEM_VMEMMAP 条件路径。 |
| `misc_mem_init(): zone_sizes_init()` | formal: `Zones.setup()` + `PageMetadataMap.setup()` | 通过 `free_area_init()` 建立 zone 层级；当前 FLATMEM 路径下 `alloc_node_mem_map()` 分配并绑定 `mem_map`，随后 `memmap_init()` 初始化 page metadata。 |
...
| `trap_init()` | formal: `ExceptionType.setup()` 和子异常 setup | 阶段终点；建立正式异常分类、分发和 handler 注册框架。 |
...
仍按四层核心语义链闭合，不得让实现反向覆盖 Charter 意图。
...
1. `内存核心初始化期对象`（暂名 `MmCoreInitPhase`）：属于阶段对象，是 `BootPhase` 的第四个子阶段对象。它从 `ExceptionType.Ready` 接续核心准备期，并以 `mm_core_init()` 返回作为阶段完成边界。
...
15. `kmalloc cache 集合对象`（正式命名为 `KmallocCaches`）：对应 `kmalloc_caches[][]` 与 `kmalloc_size_index[]`。它是 size 到 kmalloc `SlubCache` 实例的引用/索引视图，不拥有这些实例；具体 kmalloc cache 实例仍由 `SlubCacheRegistry` 注册和拥有。`SlubSubsystem` 通过该集合暴露 `kmalloc`/`kzalloc`/`kfree` facade。当前 `CONFIG_RANDOM_KMALLOC_CACHES=n` 且 `CONFIG_MEMCG=n`，只建模默认 kmalloc cache 集。
...
从这一小节开始，对应顶层 `InterruptPhase`。本阶段从 `early_irq_init()` 开始，到 `rest_init()` 创建 `kernel_init` 与 `kthreadd`、多任务开始运行之前结束。`InterruptPhase` 的首个子阶段开始时仍关闭中断，并只建立 IRQ/time 控制设施；随后独立的 `LocalIrqEnablePhase` 执行 `local_irq_enable()`。此后 boot CPU 本地中断总入口开放，常规流仍是单根启动初始化流，但已经可能被中断流打断。`rest_init()` 之后将进入 `BootInitFlow Enable`。
...
这里需要先区分早期已经建模过的 `InterruptType` 与本阶段的 IRQ 初始化对象。BootInitFlow.Preset/Setup 的 `InterruptType.preset/setup()` 已经分别用于清理中断子开关/挂起位、关闭中断总开关，并使 `InterruptType.state == Ready`；它表达的是“从常规流进入中断流的入口路径已被安全封闭”。本阶段的 `early_irq_init()` 与 `init_IRQ()` 则是在这个入口路径仍关闭的前提下，建立后续中断到来时可分发、可入栈、可路由到 handler 的控制设施。
...
- `setup()` 对应 RISC-V `init_IRQ()`，由 `InterruptType` 在中断仍关闭的上下文中驱动。它应至少推进或建立以下子对象/属性：`RiscvIrqStackSet`、可选 `RiscvIrqScsSet`、`IrqDomain` 树、关联的 `IrqChip` provider、`ArchIrqEntry`，并依次驱动 `SbiIpi.preset()` 与 `SbiIpi.setup()`。完成后要求 `IrqDispatchTree.state == Ready`，并要求 `handle_arch_irq` 已由 irqchip 路径设置；若没有中断控制器建立该入口，Linux 会 panic。
...
- `ArchIrqEntry` 表示架构 IRQ 入口已建立并可由 `handle_arch_irq` 承载。它是 `IrqDispatchTree` 的属性或关联事实，不单独推进生命周期；若后续需要细化 RISC-V exception/IRQ entry 共享入口，再与前文 `TrapType` 的正式入口关系合并讨论。
...
| `WARN(!irqs_disabled(), ...)` / `early_boot_irqs_disabled = false` / `local_irq_enable()` | next subphase: `LocalIrqEnablePhase` | 由独立子阶段确认中断仍关闭，再执行 `InterruptType.enable()`：清除 early boot 禁止标记，设置 RISC-V `sstatus.SIE` 中断总开关，打开 boot CPU 本地中断接收路径，记录 `BootCPU.local_irq_enabled == true`；`sie/sip` 仍作为具体中断类型子开关/挂起位处理，不新建 `IrqEnableGate` 对象，也不自动推进 tick/softirq/IPI 使用者到 `Online`。 |
...
- `InterruptType.state == Ready`，限定为中断入口结构已建立但 boot CPU 本地中断总入口仍关闭
...
这个边界承接 `IrqTimeInitPhase` 的出口事实：IRQ/time 控制设施已经进入 `Ready`，boot CPU 本地中断总入口仍关闭，`early_boot_irqs_disabled == true`。本阶段只通过 `InterruptType.enable()` 打开 boot CPU 的 RISC-V `sstatus.SIE` 总入口，并把 `early_boot_irqs_disabled` 清为 false；PLIC UART source gate、root supervisor external input gate、周期 tick、完整 softirq、IPI runtime、workqueue worker、RCU GP kthread、task concurrency 和 SMP concurrency 仍不得随本阶段隐式进入运行态。

本子阶段的结束状态暂定至少包含：

- `LocalIrqEnablePhase.state == Ready`
- `IrqTimeInitPhase.state == Ready`
- `InterruptType.state == Online`，限定为 boot CPU 本地中断总入口已经通过 `sstatus.SIE` 开放
...
图 23 按 Linux 6.12 `start_kernel()` 中 `pid_idr_init()` 到 `kcsan_init()` 的有效调用顺序展示 `InterruptPhase` 子阶段 4 的推进过程。绿色节点表示当前主线 formal 候选，黄色虚线节点表示保留 Linux 时序但暂不推进对象状态的 deferred 路径，橙色虚线节点表示当前配置下 trimmed/no-op 的路径。`rest_init()` 不属于本子阶段，只作为下一阶段 `BootInitFlow Enable` 的入口标记。
...
- 下一阶段的起点是 `rest_init()`，它将创建 `kernel_init` 与 `kthreadd`，并把系统推进到 `SYSTEM_SCHEDULING`；这也是从 `InterruptPhase` 转入 `BootInitFlow Enable` 的边界

### 单核多任务期对象建立

从这一小节开始，对应顶层 `BootInitFlow Enable`。本阶段从 `rest_init()` 开始，到 `kernel_init_freeable()` 中 `smp_init()` 执行前结束。阶段入口仍由上一阶段延续下来的 `BootTask` 执行，但入口动作会创建新的任务对象并打开调度运行状态；阶段末尾仍只有 boot CPU 在运行，secondary CPU 尚未启动，因此尚未进入真正多核并行。

本阶段的输入事实至少包括：

- `ProcessPreparePhase.state == Online`
- `InterruptType.state == Online`，限定为 boot CPU 本地中断总入口已经打开
...
- 因此 `PreSmpInitPhase.Started` 不能由 `BootIdleEntryPhase.Ready` 单独推出；它依赖
  `BootInitFlow.Online`、PID 1 release/dispatch 和 scheduler first handoff facts。
- `SmpRuntimePhase` 由真实 `TaskEntry::KernelInit` 入口在校验实际栈指针后启动；不得等待或假定
  `BootIdleEntryPhase` 已执行。
...
3. `BootIdleEntryPhase`：由首次调度交接后同一 BootTask/BootInitFlow 的 contextual Enter 从 schedule 返回点执行，进入 `cpu_startup_entry(CPUHP_ONLINE)` 和代表性 idle loop cycle；该子阶段完全受 `BootIdleStartupContext` 覆盖。
...
`SmpRuntimePhase` 的首个子阶段 `PreSmpInitPhase` 依赖 `BootInitFlow.Online`、
...
coordinate，不是对象或另一个 Flow。只有调度器未来恢复 BootTask 后，contextual Enter 才覆盖
...
| `user_mode_thread(kernel_init, NULL, CLONE_FS)` | formal candidate: `KernelInitTask.preset/setup/enable()` | `preset()` 形成 `TaskSpawnSpec`；`setup()` 通过 `TaskCreationCore` / `copy_process()` 建立 `KernelInitTask`（PID 1）实体；`enable()` 对应 `wake_up_new_task()`，把该 Task 放入调度器可运行集合。该 Task 随后等待 `kthreadd_done`，再进入 `PreSmpInitPhase`。 |
...
| `cpu_startup_entry(CPUHP_ONLINE)` | formal: `BootIdleEntryPhase` + idle actions | 同一 BootTask/BootInitFlow 保持身份；调度器恢复它后由 contextual Enter 从 schedule 返回点驱动入口和 idle 循环。该过程同时以 `CPUHP_ONLINE` 标记 boot CPU hotplug 状态；secondary CPU 仍未启动。 |
...
图 25 按 Linux 6.12 `rest_init()` 的有效调用顺序展示 BootInitFlow Enable、PID 1 入口和未来
BootTask 恢复分支。绿色节点表示当前主线 formal/action 候选，紫色节点表示同步门或全局状态
checkpoint，橙色虚线节点表示当前配置下 trimmed/no-op 的路径。`kernel_init_freeable()` 在
`KthreaddReadyGate.complete()` 释放并经首次 scheduler dispatch 后由 `KernelInitTask` 启动；
Boot idle 入口不构成其前置条件。
...
##### BootInitFlow Enable 子阶段 2 过程处理清单（初稿）
...
图 27 按 Linux 6.12 `kernel_init_freeable()` 中 `smp_init()` 前的有效调用顺序展示 `BootInitFlow Enable` 子阶段 2 的推进过程。绿色节点表示当前主线 formal/action 候选，黄色虚线节点表示保留 Linux 时序但暂不推进对象状态的 deferred 路径，橙色虚线节点表示当前配置下 trimmed/no-op 的路径，底部的 `smp_init()` 只作为下一顶层阶段入口标记。
...
- `BootInitScheduleHandoffPhase.state == Online`
- `BootInitFlow.state == Online`
- `BootTask` 保持同一 identity；未执行时为 Online，未来被调度恢复时通过 Dispatch 回到 OnCpu，boot CPU idle/scheduler 路径绑定同一 Task
...
2. `SmpBringupPhase`：覆盖 `smp_init()`。本轮实现重点是 Linux ordered booting 下的 BP/AP 分工：BP 创建每个 secondary CPU 自己的 idle task 与 dedicated stack，建立 CPU hotplug 同步量，通过 SBI HSM `hart_start` 释放目标 hart，并等待 AP 在独立 `ApEntryPreludePhase -> ApSmpCallinPhase -> ApOnlineIdlePhase` 中产生 `cpu_running` 与 `done_up` 确认。RISC-V secondary entry、`smp_callin()` 和 AP online-idle 入口必须作为可观测子阶段存在；完整 AP hotplug callback/offline 细节仍 deferred。完成后，多核并行语义成立。
...
| RISC-V `secondary_start_sbi` / `.Lsecondary_start_common` | AP phase: `ApEntryPreludePhase` | 目标 secondary CPU 自己执行的入口先导，可视为 BP 入口前导期的简化版。它复用 `InterruptType`、`KernelImage`、`TrapType`、`KernelAddressSpace`、`IdleTask` / `Stack` 等对象语言，但不重新清零 BSS、不重建早期页表、不重新建立 `BootCPU`；内部屏蔽中断、加载 `gp`、绑定 `tp` / `sp`、启用 MMU、设置正式异常入口和加载 SCS，然后进入 `smp_callin()`。 |
...
| `local_irq_enable()` in `smp_callin()` | AP phase: `ApOnlineIdlePhase` | AP 侧打开 RISC-V `sstatus.SIE` 本地中断总开关；和 boot CPU 的 `InterruptType.enable()` 同属 per-CPU 中断开放事实。 |
...
图 29 只展示 boot CPU / `KernelInitTask` / CPU hotplug controller 的 BP 侧流程。绿色节点表示主线 formal/action 候选，紫色节点表示 completion 等待，黄色节点表示 BP/AP 分叉点，橙色虚线节点表示当前配置下 trimmed/no-op 的路径。图中 `CpuStartProvider.start(cpu, idle)` 是关键分叉点：BP side 只负责释放目标 hart 并等待 `cpu_running` / online-idle `done_up`，不调用 AP side 的 `ApEntryPreludePhase` 或 `ApSmpCallinPhase`；图中的 `kick_ap_if_needed(target)` 表示 BP 后续唤醒目标 CPU 的 `CpuHotplugThread` 跑 AP-side callbacks，不表示 BP 直接执行这些 callback。
...
本轮 AP 侧不再整体暂缓，而是先建立三段最小 AP 启动框架：`ApEntryPreludePhase` 证明 AP 已从 `secondary_start_sbi` 进入并消费 per-AP boot data，`ApSmpCallinPhase` 证明 AP 已执行 `smp_callin()` 并 complete `cpu_running`，`ApOnlineIdlePhase` 证明 AP 已进入 `CPUHP_AP_ONLINE_IDLE` 并 complete `done_up`。这三段 checkpoint 是定位 `QEMU_SMP=2` 早期问题的长期诊断面：如果卡住，应能区分 BP 尚未发起 HSM、HSM 返回异常、AP 未到 secondary entry、AP 未消费 boot data、AP 未完成 `smp_callin()`、或 AP 已进入 online-idle 但 BP 未观察 ack。完整 AP CPU-local 对象链和 CPU hotplug 后续 callbacks 仍按以下计划补充：
...
6. 完整 live AP `CurrentTask` / `CurrentCPU` / `InterruptType` / `PreemptionControl` 解析链，当前三段 AP phase 只记录最小事实。
7. `CPUHP_AP_ONLINE_IDLE`、`CPUHP_ONLINE`、`cpu.active`、`IdleTask.Online` 与 `CpuHotplugThread` 之间的最终结束状态校准。
...
7. `用户地址空间对象`（暂名 `UserAddressSpace`）：表示每个用户态进程独立的低地址用户区映射。它是多实例对象；高地址内核映射共享或引用 `SwapperVm`。`SwapperVm` 继续表示内核共享地址空间实例，不改成普通多实例用户地址空间。首轮仅要求最小用户页表、用户页 `U` 权限、内核页 `U=0`、ELF 段映射、用户栈映射和阶段性的 heap/mmap arena；dynamic libc 首片要求同一个 `UserAddressSpace` 同时映射主程序 ELF 和 `PT_INTERP` 指向的 musl interpreter ELF，并用固定 non-overlap load bias 装载 `ET_DYN` interpreter。ELF 段的 backing/PTE 以页粒度覆盖 `align_down(p_vaddr)..align_up(p_vaddr + p_memsz)`，因此 `mprotect`/`munmap` 对动态链接器 RELRO 页保护请求的 mapped-range 判断也必须按页范围处理，而不是只按原始 `p_vaddr..p_vaddr+p_memsz` 字节范围拒绝页内前缀。heap/mmap arena 用于承接动态链接器早期 `brk`/anonymous `mmap` 需求，属于正式运行时语义，不是测试专用入口。当前文件映射首片另包含一页 transient regular file 的 writable `MAP_SHARED` VMA；该 VMA 独立持有共享 frame 和稳定 backing file 身份，所以 fd close、pathname unlink 和普通 fork 后仍可按 Linux 的映射生命周期访问同一 backing。`msync` 对该 VMA 的 `MS_SYNC` 必须把映射页内容以 positioned write 同步到仍存活的 inode，不能伪造无副作用成功或改动 opened-file position；完整 VMA 树、通用 page cache coherence、多页/partial file mapping、ASLR 和更完整 page fault recovery 后续再展开。
...
9. `用户 trap frame 对象`（暂名 `UserTrapFrame`）：表示进入 U-mode 前的寄存器现场，至少绑定 `sepc=ElfObject.runtime_entry`、用户 `sp`、`sstatus.SPP=U` 和 `SPIE=1`。静态程序的 runtime entry 是主 ELF entry；动态程序的 runtime entry 是 interpreter entry。它是固定 KernelInitFlow 的 Runtime exec commit 执行最终 trap-return handoff 的输入，并直接关联稳定 KernelInitTask。当前 RISC-V `APP=user-boot` 入口为 boot CPU 上的 PID 1 建立至少 32 KiB、64 KiB 对齐的 VMAP kernel trap stack 和 4 KiB、16 字节对齐的静态 overflow stack；每个普通用户 Task 同样拥有至少 32 KiB 的独立 kernel stack。正式响应入口必须区分异常或中断来自用户态还是内核态：用户态来源在进入通用事件、调度或 CurrentTask 路径前恢复当前 Task 身份并切换到其内核栈，返回用户态前按调度提交后的 Task 刷新入口上下文；内核态来源保持被中断的内核栈和当前 Task 身份。该入口上下文只是体系结构 lowering，不是 CurrentTask 对象、槽或 snapshot state。两种来源都必须在保存完整响应现场前确认当前栈足以容纳完整响应记录；容量不足时切换到紧急栈，保留可定位故障的完整整数寄存器及 `sepc/scause/stval/sstatus`，并以 terminal panic 停止，不得进入普通 checkpoint、分配器、printk 锁或信号路径。该首片不把不可恢复的 kernel stack overflow 转换为用户信号；per-CPU overflow stack 与 IRQ hardirq stack switch 继续 deferred。
10. `系统调用入口与表对象`（`SyscallException` / `SyscallTable`）：`SyscallException` 是 `ExceptionType` 下已有的 ecall/syscall 异常对象，负责用户态 syscall 入口、来源检查、参数提取和分发选择；不再单独建立 `SyscallDispatcher` 对象。`SyscallTable` 是独立分发表对象，承载当前支持的 syscall action 集合；具体 syscall 不是资源对象，而是 `SyscallTable.Action::Write`、`SyscallTable.Action::Writev`、`SyscallTable.Action::OpenAt`、`SyscallTable.Action::Read`、`SyscallTable.Action::Close`、`SyscallTable.Action::NewFstatAt`、`SyscallTable.Action::SchedGetAffinity`、`SyscallTable.Action::Setitimer`、`SyscallTable.Action::Kill`、`SyscallTable.Action::RtSigreturn`、`SyscallTable.Action::Brk`、`SyscallTable.Action::Mmap`、`SyscallTable.Action::Msync`、`SyscallTable.Action::Mprotect`、`SyscallTable.Action::Munmap`、`SyscallTable.Action::SetTidAddress`、`SyscallTable.Action::Exit`、`SyscallTable.Action::ExitGroup` 等 action。`write/writev` 不再直接按 fd 特判转发到 console，而是经 `FilesStruct -> FileDescriptorTable -> OpenFileDescription -> FileBackend` 解析到标准输出/标准错误对应的字符设备后端；只读 `openat/read/close/newfstatat` 首片则经 `FilesStruct` 分配一个普通文件 opened instance，并通过 VFS path read 读取当前 ext2 rootfs 中已存在的 regular file；只读 `sched_getaffinity` 在当前进程 lease 下把 `CpuGroup` active 集合按 Linux 6.12 RISC-V cpumask ABI 复制给用户态，不改变进程固定 CPU 所有权；`setitimer(ITIMER_REAL)` 在当前 aggregate 的 `SignalRuntime` 内安装、查询或取消 wall-clock timer，真实到期时发布 `SIGALRM`；正 PID `kill` 在权限允许且目标 occurrence 仍存活时向目标 aggregate 发布 pending signal，不能把成功返回与信号副作用分离；`rt_sigreturn` 只从当前 active Linux RISC-V signal frame 恢复寄存器和 mask；`brk/mmap/msync/mprotect/munmap` 路由到当前 generation/CPU-checked `UserAddressSpace`。`msync` 按 Linux 6.12 检查 flags、页对齐、长度取整/溢出和映射覆盖，非法形态返回 `EINVAL`，任一覆盖空洞返回 `ENOMEM`；`MS_SYNC` 只对 shared file VMA 执行同步，匿名/private VMA 成功但不产生文件副作用，底层同步失败不得被屏蔽。`set_tid_address` 按 Linux `current->clear_child_tid = tidptr; return task_pid_vnr(current);` 的形态落到当前 `KernelInitTask` 的 PID 1 任务属性上，不建立 persona 或 futex 完整对象。
11. `打开文件上下文对象`（`FilesStruct` / `FileDescriptorTable` / `OpenFileDescription` / `FileBackend`）：`FilesStruct` 是任务拥有的打开文件上下文，和表示 root/pwd 的 `FsStruct` 并列，不是 `FsStruct` 的下级类型。`FileDescriptorTable` 是 `FilesStruct` 内部的 fd table，负责把 fd 映射到 `OpenFileDescription`；`OpenFileDescription` 表示一次打开后的文件实例，承载 flags、offset 和后端引用；`FileBackend` 表示具体后端类型。普通 fork 复制独立 fd table，但继承项继续引用同一 live open-file description/backend；当前 pipe 首片因此以共享 backing 和跨进程 endpoint 引用维持 fork 后数据、EOF 与 close 语义，不能复制私有 pipe buffer 或依赖 parent table snapshot。阻塞式 pipe read 在 backing 为空且 writer 存活时登记当前固定 Task/CPU 并经 Scheduler sleep；首次写入可读数据或最后 writer 关闭发布合并 wake，Task 恢复后重新检查共享 backing，且 sleep 前后的 wake 不能丢失。当前支持边界分两层：第一层预安装 fd 0/1/2 为 console-like `CharDevice` 后端；第二层以有界 opened-instance 池支持只读 `RegularFile` 或 `Directory`，用于 `openat` 后的 `read`、`getdents64`、`close` 和元数据查询。只读 `openat` 的 `O_DIRECTORY` 要求最终目标为目录；`O_NOFOLLOW` 仍正常跟随中间 symlink，但最终目标为 symlink 时按 Linux 返回 `ELOOP`，不得把它当作目录或静默跟随。块设备文件、完整 `/dev/console`、权限、任意目录 dirfd 遍历、`O_PATH`、magic link、poll、`CLONE_FILES` 共享 fd table、通用写路径和 page cache 后续展开。
...
13. `信号运行期对象`（`SignalRuntime`）：表示每个用户进程 aggregate 拥有的用户态信号运行期机制，与全局初始化用 `SignalCore` 不同。当前单线程进程片明确包含 `SignalActionTable`、blocked/pending set、一个 `ITIMER_REAL` 实例及至多一个 active RISC-V signal frame；fork 复制 action/mask，但不复制 pending set、active timer 或 active frame。远程 signal producer 只能经目标 occurrence lease 和 signal mailbox lock 合并 pending bit，再把 wake 发布到目标固定 CPU，不能直接取得目标 CPU 独占的 aggregate 写引用。用户返回边界按 Linux 的 unblockable mask、`SIG_DFL`/`SIG_IGN`、action mask、`SA_NODEFER`、`SA_RESETHAND` 和 `SA_RESTART` 规则选择并递送；handler frame 使用 Linux RISC-V 的 `siginfo + ucontext` 布局，并以每个 `UserAddressSpace` 内固定的 RX/NW signal-return trampoline 进入 `rt_sigreturn`，不得使用户栈可执行。线程组共享 pending、alternate stack、实时信号排队、ptrace、job-control delivery、用户修改完整 FP/vector context 以及通用 restart-block 继续 deferred。
...
反向覆盖四层核心语义链；若后续 Ext2 对象边界发生变化，必须按所选具名工作流闭合。
...
在当前只有一个共享请求槽的收敛实现中，并发同步读调用者必须在该槽的 IRQ-safe owner 边界上串行化：已经持有 owner 的调用者负责把自己的精确 pending token 经 IRQ 或任务侧 polling 收敛并释放 owner，后继任务调用者等待 owner 后再提交自己的请求。IRQ completion 可以非阻塞地尝试取得同一 owner；若任务调用者已经持有它，IRQ 只确认设备事件并把 completion 消费留给该任务的 polling 路径。单请求槽或 completion-consumer 争用是内部调度状态，不得作为 `DeviceNotReady`、Ext2/VFS 读失败或用户可见的瞬时 `ENOENT` 返回；只有设备、transport、request 或介质本身的确定失败才可越过同步读边界。
...
`IrqHandlerRegistry`、`InterruptType` 等稳定边界上。条件编译只改变这些对象背后的 provider：使用原生 provider
...
external input gate 仍由 `arceos_ex` 既有 `InterruptType` 打开。随后显式执行 Linux PLIC UART chip callback exercise event：基于同一
...
7. 生命周期范围：CPU hotplug、syscore suspend/resume、SMP affinity 和 edge interrupt quirk 在第一轮是否纳入强等价，
   ...
   打开路径；parent `enable_percpu_irq` 目前记录 Linux 数据面但不直接控制硬件 gate，真实 root external input gate 由现有 `InterruptType` 控制。
...
分析阶段应以集合为基本单位，而不是假设成功或失败各自只有一条标准序列。首先对全部成功序列集合提取成功特征，包括共同前缀、必然出现的事件集合、可选事件集合、事件频率、关键事件相对顺序、代表序列和成功内部允许的并发交错范围。随后对每一个失败现象类别分别提取失败特征；同一个失败类别内部仍可能包含多个序列集合，报告应保留每个序列集合的出现次数、代表 run、特有事件、缺失事件和结构化 `failure_diagnostic` / `ready_check_failed` 分布。

差分诊断应执行“失败类别集合 vs 成功集合”的集合对集合比较，而不是只做单条失败日志与单条成功日志的全量文本 diff。每个失败类别至少应报告：失败类别必现但成功集合从不出现的事件、成功集合必现但失败类别缺失的事件、失败类别内各代表序列相对于最近成功序列的第一个可见分叉点、这些分叉点是否在该失败类别内收敛到同一阶段/对象/checkpoint/predicate，以及无法收敛时需要补充的观测字段。若成功集合本身存在多条合法并发序列，比较时应先扣除成功集合内部的合法扰动，再把失败集合中特有或缺失的稳定事实作为诊断候选。

首个分叉点只表示当前事件提取模型下最早可见的差异，不应被直接等同于根因。诊断报告的重点不是只统计失败率，而是把每一类失败序列集合与成功序列集合进行对比，找到当前可观测差异开始的位置，并把该位置映射回阶段、对象、checkpoint、predicate、failure diagnostic 或需要新增观测的代码路径。若当前事件序列只能看到外部症状，例如 panic、timeout 或最终 smoke 失败，就应优先补长期结构化观察点，而不是扩大临时日志或依赖全量 stdout 差分。

trace 增强应由上述差分结果驱动。如果当前 checkpoint announce/observer 和 TrapType 已能定位分叉点，则先用现有机制闭环；如果只能看到外部失败现象而无法解释内部差异，再补充更细粒度的观测点，例如 virtio-blk completion、VFS/ext2 lookup/read、initcall ready predicate、CPU/task 归属、IRQ/task context 或事件序号。Linux-like trace 是内部可见性不足时的补强方向，不是 `stress` 测试落地前的前置条件。
...
Config.Enable 和 Lds.Enable，后者依赖已经 Online 的 Config，然后分别证明 kernel ELF 与 kernel
boot artifact 的构建，两项叶事实共同推出 `kernel_boot_artifact_constructed`；`KernelImageFile`
只保留为未来文件对象名称，本轮不创建；Kernel.Ready 不包含物理装载事实。
...
InterruptType 关闭 BootCPU 的全部中断分路门控、完成一次待决中断清除写并建立相应顺序事实；该写不保证硬件驱动的待决位随后保持为零。BootInitFlow Online 后由同一固定 Flow `yields` 首次 Schedule；non-identity switch 的显式 Task/context commit 与 contextual Enter 再推进 KernelInitFlow；

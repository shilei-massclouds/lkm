...
4. `中断资源`（`InterruptType`）
   ...
   * `preset` - 屏蔽中断子开关
     ...
     * 结束状态：`InterruptType` 进入 `Prepared`，验证 `sie` 与 `sip` 确实已经清零，确保各类中断子开关与挂起状态已经被清理。

   * `setup` - 关闭中断总开关
     * 初始状态：`InterruptType` 已进入 `Prepared`，记录 `sstatus` 的当前值。
     * 执行动作：清除 `sstatus` 中控制 supervisor interrupt enable 的总开关位。这里的动作仍属于防御式过程，即即使固件交接时已经关闭中断，内核入口仍再次规范化总开关状态。
     * 结束状态：`InterruptType` 进入 `Ready`，验证 `sstatus` 的 supervisor interrupt enable 位已经清除，确保中断进入路径在子开关和总开关两个层面都被封闭。
...
8. `内核地址空间`（`KernelAddrSpace`）与控制面（`VM`）
   ...
   * `ActivateOnCpu(cpu_ref)` - 在目标 CPU 激活跳板页表
   ...
   对原始 `dtb` 的入口地址、固定头部可访问性、magic、`total_size` 下限/溢出以及 `FixMap` 槽位容量检查，属于本规格的前置证明边界；完整 blob 可访问性来自 OpenSBI handoff 契约，不伪装成逐字节物理探测。`RawDtb` 不解析 `/memory`、`/cpus` 等节点，这些内容扫描发生在后续 `EarlyDtb` / `DeviceTree` 路径。

   * `setup` - 建立早期页表

     * 初始状态：`静态对象集合.early_pg_dir` 物理地址按页对齐；内核映像可装入 `KernelImage.virt_range`；`BootArgs.dtb_pa` 指向的原始 `dtb` 已由规格前置证明确认为有效，且其完整物理范围能放入 `FixMap` 的 FDT 槽位。
...
InterruptType 清零 `sie/sip` 并建立中断封闭事实，再推进首次 Scheduler 调度和 KernelInitFlow；

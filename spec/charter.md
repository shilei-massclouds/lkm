...
- 静态实例：例如 Linux 的 `DECLARE_COMPLETION(name)`。规格层可看作 `StaticObjects` 在准备期连续驱动该实例的 `preset()` 与 `setup()`，因为静态初始化器同时提供了存储、`done = 0` 和等待队列初始结构。若该实例通过全局符号作为同步句柄暴露，`enable()` 可记录为静态发布事实或在对应 Flow 连接关系建立时完成，不必要求一个独立 Linux 调用。
...
3. `Console 对象`：覆盖 `console_init()`。这是正式 `Console` 对象的首次构造点，因此当前建模为 `Console.preset()`，使对象进入 `Prepared`。它承接已经可用的 `PrintkBuffer` 和早期 console 输出条件，先驱动 `TtyLineDisciplineRegistry.preset()`，把静态 `NTtyLineDiscipline`（Linux `n_tty_ops`）注册到 `N_TTY` slot；随后读取 `Lds/StaticObjects` 提供的 `__con_initcall_start..__con_initcall_end` console initcall 表，驱动当前配置下的 `ConsoleDriver` 对象执行 early register。该过程可能注册可用的 real console，并在 real console 成为 `CON_CONSDEV` 时触发 early/boot console handoff；但由于此时仍早于很多 bus/device probe，设备树上的真实串口 console 不保证已经完整 probe，复杂设备 probe、boot console 注销和完整 console handoff 都不能作为本阶段必然后置条件。
...
| `console_init()` | formal candidate: `Console.preset()` | 首次构造正式 `Console`：注册 `NTtyLineDiscipline` 到 `TtyLineDisciplineRegistry`，遍历 `Lds/StaticObjects` 提供的 console initcall 表，驱动 console drivers early register；real console 注册和 early/boot console handoff 是条件结果，不作为必然后置条件。 |
...
9. `pre-SMP initcall 表驱动`（暂名 `PreSmpInitcallTable`）：覆盖 `do_pre_smp_initcalls()`。它遍历 linker/StaticObjects 提供的 `__initcall_start` 到 `__initcall0_start` 区间；链接脚本先放 `.initcallearly.init`，随后才进入 `.initcall0.init`，因此本调用只执行 `early_initcall()`，不执行后续 `pure_initcall()` / `core_initcall()` 等 level。规格层把它建模为 `PreSmpInitcallTable.run_early()` action，而不是独立生命周期对象；它驱动 `RcuCore`、`Softirq`、`Scheduler`、`CpuStopper`、`AddressSpaceId`、`MemoryZeroPage` 与 `CompatMode` 等已有或候选对象完成早期动作。其它 sysctl/proc/debug/tracing/模块相关 early initcall 按当前配置和重要性记录为 deferred 或 trimmed。
...
1. `InitcallTable`：独立静态表对象，依赖 `StaticObjects` 中的 `__initcall0_start` 到 `__initcall_end` linker ranges。核心 action 是 `run_all_levels()`，完成后记录 `all_levels_ran == true`。
...
3. `RootFS.enable()`：对应当前 `prepare_namespace()`。它在 `init_eaccess(ramdisk_execute_command)` required checkpoint 成立后执行，内部包含从初始 `RamFS`/`rootfs` backing 到真实根文件系统 backing 的 handoff-like 切换；完成后 `RootFS.state == Online`，用户态入口可基于它查找 init 候选。当前轮次该 enable 细节标记为 deferred。
...
| `prepare_namespace()` | deferred: `RootFS.enable()` | `RootFS` 是顶层根文件系统对象；enable 内部包含从初始 `RamFS`/`rootfs` backing 到真实根文件系统 backing 的 handoff-like 切换。当前不展开 root device probe、fs type 选择、mount fallback、initrd/NFS/CIFS 等细节。 |
...
- `RootFS.enable()` 保留时序位置但当前轮次标记为 deferred；`init_eaccess(ramdisk_execute_command)` 必须满足调用 `prepare_namespace()` 的条件
...
- `PayloadParam.state == Ready`
- Linux-like 路径下，`RootFS` 至少提供初始 rootfs 视图；`RootFS.enable()` 和真实根切换细节当前保留为 deferred，不作为本轮强完成条件
...
仍由 `arceos_ex` 既有 `InterruptStream` 打开。`linux_plic.boundary_facts` checkpoint 还会基于同一 UART leaf
`irq_data` 受控调用 Linux `plic_chip.irq_disable -> irq_enable -> irq_mask -> irq_unmask`，以观察黑盒 chip callback
服务面和交换数据布局是否可用；该探测最终恢复为 enabled/unmasked 状态，不改变后续 UART IRQ 闭合路径。随后 flow handler 进入 `handle_fasteoi_irq`：先恢复 Rust `tp` 分发
...
callback 已经承接现有 UART action；同一 checkpoint 输出还约束了 `irq_enable/irq_disable/irq_mask/irq_unmask`
callback 至少各成功执行一次。

这仍不是完整 Linux generic IRQ core 复用。当前只覆盖 QEMU/SiFive PLIC 的无 edge quirk、level IRQ 主线；
`handle_edge_irq` 仍只是兼容入口，`plic_edge_chip.irq_ack`、disabled IRQ eoi 特殊路径、unmapped IRQ
ratelimit/打印以及更完整 `irq_desc`/`irq_common_data` 行为仍归入后续排雷。`mask/unmask/disable` 目前只属于
checkpoint 主动探测的 callback service 面覆盖；若后续接入完整 Linux generic IRQ lifecycle，还需要再补真实路径语义。
...
2. 原生 PLIC 的对等上接口：需要决定 `arceos_ex` 原生 PLIC 是否也暴露 platform-driver-like `probe`、`irq_domain_ops`、
   `irq_chip` 和 chained handler 形态，或者只在内部通过极薄 provider wrapper 对齐到同一 contract。目标是条件编译
   切换时，上层 `IrqDispatchTree/RiscvIntc/IrqHandlerRegistry` 等对象不发生语义变化。
...
5. IRQ flow 与 complete 位置：level IRQ 主线已通过 `handle_fasteoi_irq -> plic_chip.irq_eoi` 接入 Linux-shaped
   complete；`plic_chip.irq_enable/irq_disable/irq_mask/irq_unmask` 已先通过受控 checkpoint probe 验证 callback
   service 面可达。后续仍需补 edge quirk 下的 `handle_edge_irq -> plic_edge_chip.irq_ack`、disabled IRQ 特殊 eoi
   路径、unmapped IRQ fail/ratelimit，以及更完整 `irq_desc`/`irq_common_data` 语义，保证

...
事实匹配后才 dispatch。这个对齐仍是最小 request/action 数据面，不表示已经实现完整 Linux `free_irq/threaded`
生命周期。
...
事实的最小对齐，以及 native/linux-object provider 共享的薄 `objects::plic_provider` contract。显式

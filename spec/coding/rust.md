...
- 内核实现只默认信任 Rust 提供的系统 crates 和本项目自己建立、维护的 crates。
- 发现可借用的外部 crate 时，不得把它作为黑盒依赖直接引入内核实现。可以参考其机制重新实现，也可以把源码引入
...
- 例外仅限 Rust 系统 crates；其它例外必须在 coding 规格中显式记录原因、风险和替代方案。

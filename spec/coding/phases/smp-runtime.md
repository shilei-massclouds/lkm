...
每个叶阶段的 Online continuation 只能返回 KernelInitFlow 当前正文 coordinate。初始与恢复入口都必须
先由 Scheduler 提交 active CurrentTask binding。所有入口和 continuation 都必须即时验证
KernelInitTask OnCpu、`CurrentTask` 解析为 KernelInitTask，并验证当前 SP 位于

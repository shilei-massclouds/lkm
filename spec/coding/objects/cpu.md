...
- Keep CPU-local `LocalInterruptControl`, `CurrentTaskSlot`, and related local state inside `Cpu`.
...
- Lower `CurrentCPU` as a stateless `CurrentCpu` borrow created from an effective `TaskFlow` plus `CpuGroup`.
  It cannot be constructed from a Task, a raw logical ID, or an asynchronous emission.

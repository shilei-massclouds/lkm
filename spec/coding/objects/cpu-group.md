...
hartid uniqueness and child construction before publishing the element or changing parent facts. Failure leaves
the stable collection and all derived masks unchanged. CPU0 must be created and reach Prepared before the kernel
Enable handoff. `setup_smp()` creates AP elements through the same operation.

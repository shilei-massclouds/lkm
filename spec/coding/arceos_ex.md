...
- `user-smoke` 应包含独立的 stdin syscall case，用于验证 fd0 `read(63)` ready-data 首片。该 case 只能消费默认 overlay 进入用户态前准备的固定 ready data，并输出清晰的 `syscall read stdin ok` marker；它不得依赖人工键盘输入、第二个用户任务或真实 blocking read 前进，也不得把这个 case 移入 `fileio` 的 regular/directory VFS 回归中。

...
- `rt_sigtimedwait(137)` 当前只新增 unsupported 诊断，不进入 supported syscall 分发表，也不新增 observed/supported facts。本地 Linux 6.12 `include/uapi/asm-generic/unistd.h` 明确 `__NR_rt_sigsuspend=133`、`__NR_rt_sigtimedwait=137`；不得把 OpenRC 当前触达的 `137` 误标为 `rt_sigsuspend`。原生 `/sbin/init` 诊断在 `setsid(157)` 后反复到达 `nr=137`，参数形态为 `uthese=a0=0x100c9350`、`uinfo=a1=NULL`、`uts=a2=NULL`、`sigsetsize=a3=8`，返回仍为 `-ENOSYS`。unsupported diagnostic 可以打印 `name=rt_sigtimedwait`、`uthese/uinfo/uts/sigsetsize`，并对 `uthese` mask 和非空 `uts` timespec 做映射保护下的 best-effort 只读复制；复制失败、空指针或跳过必须以稳定 token 表示。该诊断不得写用户内存、不得改变 checkpoint 顺序、不得改变 `ENOSYS` 返回、不得把 syscall 计为已支持。对照 Linux signal wait 语义，`uts=NULL` 且无 pending matching signal 时会等待信号，不能伪装成成功或零超时 `EAGAIN`；零超时 `EAGAIN`、有限 timeout、pending queue selection、`siginfo_t` copyout、真实 wait/scheduler sleep、signal delivery 和 restart syscall 均留给后续按证据规格化。

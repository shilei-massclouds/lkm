...
credential/tty ownership 调整中的 `fchown(55)` / `fchmod(52)`：复现证据显示 BusyBox login 认证后先调用
`fchown(fd=0, uid=1000, gid=100)`，再调用 `fchmod(fd=0, mode=0600)`，guest 文本仍可能概括为
`login: can't set groups: Function not implemented`。该文本不得被解释为首个缺失 syscall 是 `setgroups(159)`；
本地 asm-generic 头文件确认 `52=fchmod`、`55=fchown`、`159=setgroups`。当前片只闭合 fd-local
`fchown/fchmod`：必须经 `FilesStruct -> FileDescriptorTable` lookup 验证已打开 fd，成功时在 fd entry 记录
owner uid/gid 和 chmod mode override，坏 fd 返回 `EBADF`；`fchmod` 后同 fd 的 `fstat` 至少要反映 permission
mode override。不得实现或声称 inode ownership/mode 持久化、ext2 writeback、权限/capability/LSM、TTY ownership
模型、path chmod/chown、idmapped mount、user namespace 或 `setgroups(159)`。OpenRC login shell case 仍只能作为 opt-in

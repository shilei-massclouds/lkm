...
- stdin `read(63)` ready-data 首片以 Linux 6.12 `fs/read_write.c::ksys_read()/vfs_read()`、`fs/file.c::fdget_pos()`、`drivers/tty/tty_io.c::tty_read()`、`drivers/tty/n_tty.c::n_tty_read()` 和 RISC-V `include/uapi/asm-generic/unistd.h` 的 `__NR_read=63` 为基准。当前只支持 fd0 char-device 在已有 bounded ready data 时的成功路径：`SyscallTable.Action::Read` 必须经 `FilesStruct -> FileDescriptorTable -> OpenFileDescription -> FileBackend::CharDevice -> TtyFlipBuffer` 复制数据，不得在 syscall table 中直接写测试字符串。选中的 ELF 是受控 `user_smoke` fixture 时，可以在进入用户态前准备固定 ready-data fixture，用于默认 overlay smoke 和 `make test` 的临时 requested-init overlay 验收；真实发行版 init/sh/ls ELF 不得注入该 fixture。fd0 没有 ready data 时的真实阻塞语义、canonical N_TTY 行缓冲、wait queue/wakeup、job control、controlling tty、poll/ppoll 阻塞等待、signal interruption/restart、termios 和 IRQ RX 唤醒继续 deferred/trimmed，不得把返回 EOF/EAGAIN 的临时行为声称为 Linux blocking read 完成。`PROBE=user-read-trace` 是显式低噪声 read 结果诊断，用于定位发行版 `/bin/sh` 在 prompt 后退出但没有 syscall error 的情况；它必须在不改变 read 返回值、errno 映射、checkpoint 序列、user-smoke 判定和默认 `make test` 输出的前提下，打印每次 supported `read(63)` 的 fd、requested length、`USER_COPY_MAX` capped length、成功返回值或失败原因、trap mode、`a0..a2`、`sepc` 和 `stval`，必要时打印内部 `FileError` 或 copy-to-user 失败原因。该 probe 只能作为定位证据，不能把当前 no-ready-data 返回 0 解释为 Linux blocking TTY 语义已经完成。
...
- `user-smoke` 应包含独立的 stdin syscall case，用于验证 fd0 `read(63)` ready-data 首片，并在同一固定 ready data 被消费前验证 `ppoll(73)` 的立即 readiness 首片。该 case 只能消费默认 overlay 进入用户态前准备的固定 ready data，并输出清晰的 `syscall ppoll stdin ok` 与 `syscall read stdin ok` marker；它不得依赖人工键盘输入、第二个用户任务或真实 blocking read 前进，也不得把这个 case 移入 `fileio` 的 regular/directory VFS 回归中。

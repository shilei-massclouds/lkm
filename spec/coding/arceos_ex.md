...
- Linux `PayloadPhase.Online` paired anchor 当前位于默认 fallback 的 `try_to_run_init_process("/sbin/init")`
  block 前；`init=` requested-init 成功路径会在该 block 前返回。因此使用显式 `init=` 的 paired case 不得把
  `PayloadPhase.Online` 放入 hard diff scope，除非 Linux 插桩先补充 requested-init 成功路径的同等稳定 anchor。

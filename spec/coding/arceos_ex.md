...
正式硬约束位于 [`arceos_ex.spec`](arceos_ex.spec)，并由 [`main.spec`](main.spec) 统一 include。本文承载解释、背景、参考路径、阶段性取舍和 predicate 说明；不得用本文覆盖 `.spec` 中的 rule ID 或硬约束层级。
...
以下说明从 `spec/coding/arceos_ex.spec` 的长注释迁移而来。`*.spec` 文件只保留 rule ID、type 分组、MUST/SHOULD/MAY/NOTE 层级和最短标签；解释、背景、参考路径、阶段性取舍与例子在这里维护。

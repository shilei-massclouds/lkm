...
`socket(198)` 诊断分类片：unsupported diagnostic 必须在仍返回 `ENOSYS` 的前提下打印 syscall name、raw
`domain/type/protocol`、去掉 `SOCK_CLOEXEC` / `SOCK_NONBLOCK` 后的 base type，以及是否为
`AF_UNIX + SOCK_DGRAM` syslog-like 形态。复跑分类结果：早期 `socket(AF_UNIX, SOCK_DGRAM|SOCK_CLOEXEC, 0)`
返回 `ENOSYS` 后还能继续到 `sendto(206)`，属于 tolerated/noisy path；post-auth `fchmod` 后的直接阻断是
...
本片不得安装 socket fd、不得实现 AF_UNIX/backend/network、`sendto(206)`、`connect`、`setgroups(159)`、完整
credentials、TTY ownership 或 inode ownership/mode 持久化。OpenRC login shell case 仍只能作为 opt-in
...
- Unsupported syscall 首片必须参照 Linux 6.12 RISC-V 路径：`arch/riscv/kernel/traps.c::do_trap_ecall_u()` 在 syscall 分发表前预置 `a0=-ENOSYS`，`arch/riscv/kernel/syscall_table.c` 将未填表项指向 `__riscv_sys_ni_syscall`，`kernel/sys_ni.c::sys_ni_syscall()` 返回 `-ENOSYS`。当前实现对未知 syscall number 或本项目尚未支持的 syscall 子路径必须返回 `-ENOSYS`，并打印稳定诊断行，至少包含 `nr`、trap mode、`a0..a5`、`sepc` 和 `stval`；该诊断用于定位下一批 syscall 缺口，不表示该 syscall 已建模支持，不得新增 checkpoint 或改变成功路径事件序列。OpenRC/login 早期证据中的 `socket(198)` / `sendto(206)` 返回 `ENOSYS` 后用户态仍能推进到 getty/login，因此该阶段只把它们记录为 tolerated/noisy unsupported syscall；fd-local `fchown(55)` / `fchmod(52)` 闭合后的 focused trace 又记录 post-auth 第一 unsupported 为 `socket(198)`，而不是 guest 文本中的 `setgroups(159)`。当前片只做这个 post-auth `socket(198)` 的诊断分类：`print_syscall_name()` 必须覆盖 `socket`，若后续 trace 到达 `sendto(206)` 也必须能显示 `sendto`；`socket(domain, type, protocol)` unsupported detail 必须打印 raw domain/type/protocol、`type` 去掉 `SOCK_CLOEXEC`/`SOCK_NONBLOCK` 后的 base type、两个 flag 是否存在，以及是否为 `AF_UNIX + SOCK_DGRAM` syslog-like 形态。该 detail 仍必须返回 `ENOSYS`，不得安装 fd、改变用户内存、改变 checkpoint 顺序或提前实现 AF_UNIX/backend/network、`sendto(206)`、`connect`、`setgroups(159)`、完整 credentials 或 TTY ownership。本片 focused run 已显示 post-auth `socket(AF_UNIX, SOCK_STREAM|SOCK_CLOEXEC, 0)` 返回 `ENOSYS` 后未继续到 `setgroups(159)`，而是立即进入 login failure/`exit_group(1)` 路径，因此分类为直接阻断；下一片若继续推进 login shell，应先规格化最小 AF_UNIX/socket 首片。若真实发行版路径到达 `nanosleep(101)`，诊断可以在同一行或紧邻行增加 `rqtp`、`rmtp`、`req_sec`、`req_nsec` 或 `req_copy=failed`，但该 best-effort 用户内存读取只用于定位，不得改变 `ENOSYS`、用户内存、checkpoint 顺序或后续 syscall 顺序。若真实 `/bin/sh -> ls` child continuation 到达 `execve(221)`，unsupported 诊断必须在仍返回 `ENOSYS` 的前提下增加 `name=execve`、`filename_ptr`、`argv_ptr`、`envp_ptr`、是否已经进入 `UserChildProcess` continuation，以及受 `UserAddressSpace` 映射范围保护的 best-effort filename、bounded argv/envp 前缀指针和值复制；复制失败、空指针或跳过必须以稳定 token 表示，不得触发新的 page fault、改变用户内存、改变 checkpoint 顺序、改变 `ENOSYS` 返回或把该 syscall 计为已支持。

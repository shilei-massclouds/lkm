...
- 加入 bounded stack snapshot/restore 后，同一 `/bin/sh -> ls` guest 已越过上述 instruction page fault；新的观察边界为 BusyBox `/bin/sh` 报 `can't set tty process group: No such process` 并以 `user exit status=2` 退出。后续继续推进前，必须先对照 Linux 6.12 的 `setpgid/getpgid`、`TIOCGPGRP/TIOCSPGRP`、session/controlling tty 和 foreground process group 路径定位直接 `ESRCH` 来源，不能从用户态错误文本直接猜修。
- `getpgid(155)` / `setpgid(154)` 进程身份首片以 Linux 6.12 `kernel/sys.c::do_getpgid()/sys_getpgid()/sys_setpgid()` 和 RISC-V `include/uapi/asm-generic/unistd.h::__NR_getpgid=155` / `__NR_setpgid=154` 为基准。`getpgid(0)` 必须读取当前 `UserInitProcess` 的 process group；当前单 PID1 首片返回 1。`getpgid(1)` 可作为同一 PID1 查找成功返回 1；其它正数 pid 或用户态传入的负数 pid 在当前无 task graph 的首片中按 Linux 查找失败形状返回 `ESRCH`。`setpgid` 必须保留 Linux 的 pid/pgid 零值归一化：`pid == 0` 表示当前 PID1，`pgid == 0` 表示归一化后的 pid；当前首片只接受 `setpgid(0,0)`、`setpgid(0,1)`、`setpgid(1,0)` 和 `setpgid(1,1)`，都保持 PID1 的 pgrp 为 1 并返回 0。负 `pgid` 返回 `EINVAL`，未知 `pid` 返回 `ESRCH`，未知目标 pgrp 返回 `EPERM`。该 syscall 读取/更新 `UserInitProcess` 的 process-group/session 事实，不落入 `FilesStruct` 或 TTY ioctl；完整 tasklist/RCU、`security_task_getpgid()` / `security_task_setpgid()`、pid namespace、`PF_FORKNOEXEC/EACCES`、多进程 process group、`setsid` 和 job-control signal delivery 继续 deferred。

...
- OpenRC child record 回收后，`make run APP=user-boot ROOTFS_OVERLAY=none QEMU_APPEND=earlycon=sbi PROBE=user-syscall-trace,user-syscall-error FORCE=1` 的可复现边界已越过 `/dev/ttyN` `openat(AT_FDCWD, "/dev/ttyN", O_RDWR|O_NONBLOCK|O_LARGEFILE)` 旧的 `access_mode+unsupported_flags` 拒绝；最新直接边界是首个 `/dev/tty1` 成功安装 fd 后，OpenRC 调用 `fcntl(fd=3, F_SETFL=4, arg=0x8002 /* O_RDWR|O_NONBLOCK|O_LARGEFILE */)` 并因尚未支持 `F_SETFL` 返回 `EINVAL`。本片只解除该 TTY status flag 更新边界：`/dev/tty[0-9]+` 仍是现有 console-like `tty0` 后端的阶段性别名，`F_SETFL` 只允许当前 console-like TTY char-device fd 更新持久 file status flags 中的 `O_NONBLOCK` 位。若通过该 fcntl 后进入 `/dev/ttyN` 单 fd/AlreadyOpen、`/dev/null`、`ioctl`、`read`、`ppoll`、`nanosleep`、`wait` 或其它 syscall 新边界，必须保留精确诊断并在下一片基于证据规格化；不得在本片引入完整 devtmpfs、真实 `/dev/console`、多个 TTY 实例、TTY driver registry、controlling tty 分配、N_TTY 非阻塞读完整语义或 job-control 完整语义。

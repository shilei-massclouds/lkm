...
- OpenRC child record 回收后，`make run APP=user-boot ROOTFS_OVERLAY=none QEMU_APPEND=earlycon=sbi PROBE=user-syscall-trace,user-syscall-error FORCE=1` 的可复现边界已推进到反复 `openat(AT_FDCWD, "/dev/ttyN", O_RDWR|O_NONBLOCK|O_LARGEFILE)`：诊断行形态为 `syscall openat reject reason=access_mode+unsupported_flags flags=0x8802 unsupported_flags=0x800 access_mode=0x2 path="/dev/tty1"`，随后同形态触达 `/dev/tty2` 并输出 `can't open /dev/ttyN: Invalid argument`。本片只解除该 TTY open 兼容边界：`/dev/tty[0-9]+` 是现有 console-like `tty0` 后端的阶段性别名，`O_NONBLOCK` 只作为 accepted TTY fd 的持久 file status flag 保存以便 `F_GETFL` 或后续 read/poll 诊断观察。若通过该 open 后进入 `ioctl`、`read`、`ppoll`、`nanosleep` 或其它 syscall 新边界，必须保留精确诊断并在下一片基于证据规格化；不得在本片引入完整 devtmpfs、真实 `/dev/console`、多个 TTY 实例、TTY driver registry、controlling tty 分配、N_TTY 非阻塞读完整语义或 job-control 完整语义。
...
- Fd API 兼容首片以 Linux 6.12 `fs/fcntl.c::sys_fcntl()/do_fcntl()`、`fs/read_write.c::ksys_lseek()` 和 RISC-V `include/uapi/asm-generic/unistd.h` 的 `__NR_fcntl=25`、`__NR_lseek=62` 为基准。当前支持 `fcntl(fd, F_GETFL, 0)`、`F_GETFD` 和 `F_SETFD`。`F_GETFL` 必须通过 fd table 找到 open file description，并返回打开时记录的 Linux open flags：stdio 返回其访问模式，regular/directory fd 返回 `O_RDONLY` 以及打开时接受的 `O_DIRECTORY` 等持久 flags，TTY fd 返回 accepted open 中的 `O_RDWR|O_LARGEFILE|O_NONBLOCK` 等持久 status flags，且不得返回 `O_CLOEXEC`。`F_GETFD` 返回 `FD_CLOEXEC` 或 0；`F_SETFD` 只取 `arg & FD_CLOEXEC` 修改同一 fd entry 的 close-on-exec bit。无效 fd 返回 `EBADF`，未知 cmd 返回 `EINVAL`。`lseek(fd, offset, SEEK_SET/SEEK_CUR/SEEK_END)` 必须通过 `FilesStruct -> FileDescriptorTable -> OpenFileDescription` 消费和更新当前 file position；regular file 的 `SEEK_END` 基于文件 size，directory fd 的 offset 是当前 `getdents64` byte offset，stdio/不可定位 fd 返回 `ESPIPE`。负目标 offset、未知 whence 返回 `EINVAL`。本首片仍裁剪 `F_SETFL`、dup、file locks、owner/signals、leases、pipe/memfd fcntl、O_APPEND/O_NONBLOCK 可变语义、完整 `vfs_llseek()` per-file op、64-bit overflow、concurrent `fdget_pos()` locking/refcount 和权限/LSM；这些必须作为 deferred/trimmed 语义保留。

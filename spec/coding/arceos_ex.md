...
`setgroups(159)` 返回 0；新的直接边界是 `setgid(144)`，参数为 `gid=100`，当前最小 root setgid slice
仍按 out-of-slice 形态返回 `EPERM`，login 打印 `setgid: Operation not permitted`。本片只记录该证据，
不得顺手扩大 setgid/credentials/TTY 语义。OpenRC login shell case 仍只能作为 opt-in
...
- 当前 setgroups 更新：OpenRC login 已越过 post-auth `socket(198)` 和 `/var/run/nscd/socket` `connect(203)` missing-path `ENOENT`，本片已闭合 root-only `setgroups(159)` 首片；focused 复跑后的新直接边界是 `setgid(144)`，参数为 `gid=100` 并返回 `EPERM`。nested-vfork、fd-local metadata 与 setgroups 首片仍不包含完整 socket/network、完整 supplementary groups/credentials 切换、setgid 扩展或 TTY ownership 语义。

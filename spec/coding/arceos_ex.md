...
marker 后发送对应测试密码。不得通过内核绕过认证。当前 focused run 已验证多阶段输入和账号 overlay 能越过认证，新的边界是
BusyBox login 认证后调用 `clone(220)` vfork，而当前单 active child slot 仍被 OpenRC/getty child 占用并返回 `ENOSYS`；
因此 OpenRC login shell case 在 nested vfork/child-slot 语义规格化前只能作为 opt-in 诊断入口，不能进入默认 `make test`
硬门禁。nested vfork 闭合后，该入口再恢复为 shell prompt 后执行 `/bin/ls`、断言 `lost+found` 和 `user exit status=0`
的验收。

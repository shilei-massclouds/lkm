...
- OpenRC child record 回收、`F_SETFL` 首片和 TTY alias 多 fd entry 首片后，`make run APP=user-boot ROOTFS_OVERLAY=none QEMU_APPEND=earlycon=sbi PROBE=user-syscall-trace,user-syscall-error FORCE=1` 的可复现边界已越过 `/dev/ttyN` `openat(AT_FDCWD, "/dev/ttyN", O_RDWR|O_NONBLOCK|O_LARGEFILE)` 旧的 `access_mode+unsupported_flags` 拒绝、首个 TTY fd 上的 `fcntl(F_SETFL, O_NONBLOCK)`，以及后续 `/dev/tty2`、`/dev/tty3` 等 alias open 的 fd 3 单槽 `FileError::AlreadyOpen`/`EMFILE` 阻断。当前直接阻断是 OpenRC 继续打开更多 TTY alias 后耗尽固定 fd table，返回 `FileError::TooManyOpenFiles`/`EMFILE`，OpenRC 仍报 `can't open /dev/ttyN: No file descriptors available`。本片行为只解除 TTY alias 单 fd 槽边界：`/dev/tty[0-9]+` 仍是现有 console-like `tty0` 后端的阶段性别名，打开时只在固定 fd table 的 3..15 范围选择首个空闲 fd entry，各 fd entry 的 status flags 独立，后端仍共享同一个 `Tty0` console-like OFD/设备能力；本片不扩容 fd table。若之后进入 fd 生命周期/关闭继承、`/dev/null`、stdio close、`ioctl`、`read`、`ppoll`、`nanosleep`、`wait` 或其它 syscall 新边界，必须保留精确诊断并在下一片基于证据规格化；不得在本片引入完整 devtmpfs、真实 `/dev/console`、多个 TTY 实例、TTY driver registry、controlling tty 分配、N_TTY 非阻塞读完整语义或 job-control 完整语义。

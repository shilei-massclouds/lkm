...
`.hushlogin` missing-path 和 MOTD 输出；新的 fatal 边界是登录 shell `execve(221)` 返回 `EFAULT`，
用户态打印 `login: can't execute '/bin/sh': Bad address`。该 `execve` EFAULT 只作为后续片证据记录，
本片不得顺手修复。

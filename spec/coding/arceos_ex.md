...
本片预期在当前 rootfs 中返回 `ENOENT` 并让 libc/NSS fallback 暴露下一条真实边界。OpenRC login shell case 仍只能作为 opt-in

...
`clone_vfork stage=copy_user_process` / `login: vfork: Function not implemented`。新的第一边界是登录后
credential/tty ownership 调整中的 unsupported `nr=55`（本地 asm-generic 头文件中为 `fchown`），guest 文本为
`login: can't set groups: Function not implemented`，尚未到 shell prompt。OpenRC login shell case 仍只能作为 opt-in
诊断入口，不能进入默认 `make test` 硬门禁；下一片必须先规格化该新 syscall/credential 边界，不得在本片扩大网络、
完整 task graph 或 credentials/TTY ownership 范围。

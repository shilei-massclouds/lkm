...
`NGROUPS_MAX`。该片不实现 `getgroups(158)`、完整 `group_info` 分配/排序、capabilities、user namespace、
...
`setuid(146)`，参数为 `uid=1000`，当前仍返回 `EPERM` 并打印 `login: setuid: Operation not permitted`；
本片只记录该证据，不得顺手实现 capabilities、user namespace、LSM、task graph credential COW/RCU、
`setuid(1000)`、权限矩阵、TTY ownership 或 inode ownership/mode 语义。OpenRC login shell case 仍只能作为 opt-in
诊断入口，不能进入默认 `make test` 硬门禁；下一片必须先规格化该 `setuid(1000)` 边界。
...
- 当前 setgid 更新：OpenRC login 已越过 post-auth `socket(198)`、`/var/run/nscd/socket` `connect(203)` missing-path `ENOENT` 和 root-only `setgroups(159)` 首片；focused 复跑后的直接边界是 `setgid(144)`，参数为 `gid=100`。本片只在当前 effective uid 为 0 时同步 `UserInitProcess.gid/egid/sgid/fsgid` 到目标 gid，并保留完整 capabilities、user namespace、LSM、credential COW/RCU、权限检查、TTY ownership 和 inode ownership/mode 语义为 deferred。focused 复跑确认 `setgid(144, gid=100)` 返回 0 后，新的直接边界是 `setuid(146, uid=1000)` 返回 `EPERM`；本片只记录该证据。
...
- Credentials 首片以 Linux 6.12 `kernel/sys.c::__sys_setuid()/__sys_setgid()/sys_getuid()/sys_getgid()`、`kernel/cred.c::prepare_creds()/commit_creds()` 和 RISC-V `include/uapi/asm-generic/unistd.h` 为基准。`getuid()` 和 `getgid()` 必须从当前 `UserInitProcess` credentials 子状态返回当前 real uid/gid，当前 root init 初值为 0。`setuid(uid)` 和 `setgid(gid)` 必须路由到同一子状态；`setuid` 第一片仍只接受 BusyBox 启动路径所需的 `uid == 0` 并保持 real/effective/saved/fs uid 同步为 0，当前 OpenRC post-setgid `setuid(1000)` 仍是返回 `EPERM` 的下一边界；`setgid` 第一片在 credentials ready 且当前 effective uid 为 0 时接受 observed 32-bit gid 目标，并同步 real/effective/saved/fs gid。当前以 `euid == 0` 作为阶段性 `CAP_SETGID` 代理，不实现 capabilities、user namespace、LSM、`prepare_creds()` 分配失败、`set_user()`/ucounts、RCU/cred refcount、完整 saved-id 权限矩阵、`setuid(1000)` 或其它权限检查；非 root effective uid 的 `setgid` 返回 `EPERM`，无效 id 后续再补完整 `EINVAL` 范围规则。

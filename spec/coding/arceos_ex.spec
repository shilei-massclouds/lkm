...
type ArceosExIrqTimeInitCodingMust {
    invariant {
         ...
         * whole-line marker comments generated by this project, the mapping
         * parser must ignore those LKM_CHECKPOINT lines in its in-memory view
         * before resolving symbols, anchors and line numbers. This keeps the
         * tracked mapping artifacts stable across clean and marker-annotated
         * Linux trees, and it must not replace the explicit marker
         * stale/mismatch check path. User-mode syscall checkpoints may map to the

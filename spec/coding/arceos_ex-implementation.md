...
`make test-checkpoints`，除非参考 Linux tree 已成为仓库内受控产物。根目录 `make checkpoints-linux-check` 必须以
`--check --check-markers` 同时只读校验 tracked plan 与 sibling Linux markers；`make difftest` 必须先串行通过
`test-checkpoints` 和 `checkpoints-linux-check`，失败时只提示人工同步，不得自动运行 `make checkpoints`、生成 patch
或修改 Linux tree。
...
- Linux `PayloadPhase.Online` 表示 selected payload 的 `kernel_execve()` 已成功完成，不表示仅开始尝试 handoff。
  Linux runtime 插桩必须在 `run_init_process()` 中检查 `kernel_execve()` 成功返回后记录，从而统一覆盖 ramdisk、
  `init=` requested-init、`CONFIG_DEFAULT_INIT` 和固定 fallback 变种；任一失败返回都不得记录 Online。规范 exact
  mapping anchor 位于这一共享成功边界，运行序列必须保持 `UserBoot.* -> UserAddressSpace.Ready ->
  PayloadPhase.Online -> UserInitProcess.EnterUserMode`。

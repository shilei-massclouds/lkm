...
本节列出对象级代码生成和实现阶段必须遵守的硬性要求。它们按优先级排列；后续小节和其它 coding 文档不得降低这些要求。若无法满足任一条，必须停止实现并报告规格缺口、工具缺口或实现阻塞。

1. **模型优先**：实现必须严格遵循 `spec/model` 推导出的对象、状态、事件、依赖、驱动顺序、阶段边界和证明义务。编码便利、目录习惯、复用既有代码、提前进入 Rust、减少汇编或构建工具限制，都不能作为偏离模型语义的理由。
2. **先映射后编码**：生成或修改代码前，必须先确定模型对象到源码文件的映射表。代码只能写入该映射表指定的文件或其显式子模块；若必须偏离，必须先在 mapping 规格或当前实现计划中记录例外、原因、生效范围和保留的模型边界。
3. **落点必须对应对象所有权**：模型对象的主体实现必须放在该对象对应的文件中。Phase 对象的 `setup()`、`handoff()`、边界检查和 checkpoint 必须放在该 Phase 文件中；资源对象的状态和事件方法必须放在对应资源对象文件中。不得把子 Phase 或资源对象主体逻辑放入父对象文件后再用注释解释。
4. **Phase 目录必须反映阶段树**：Phase 文件路径必须按模型父子关系组织。父 Phase 对应目录或 `mod.rs`，直接子 Phase 位于父 Phase 目录下。若阶段树变化，源码目录必须随之调整，除非明确登记为临时例外。
5. **Phase 必须按 DFS 生成**：Phase 代码生成必须以模型阶段树为输入，按深度优先遍历生成。父 Phase 只提供子 Phase 顺序和完成确认；兄弟 Phase 之间的运行时控制流由 `handoff() -> next.setup()` 显式连接，不得生成父 Phase 直接逐个调用所有子 Phase 的串行主体。
6. **Phase 不得伪装为资源对象**：Phase 对象不得生成资源对象式 `struct + impl`，不得使用普通对象式 `Lifecycle` 状态机承载事件推进。需要状态差分或父阶段确认时，只能使用轻量边界状态记录，且该记录不得替代 `setup()/handoff()` 调用结构和 checkpoint/check 函数。
7. **事件边界不可合并**：每个模型事件必须在代码中有可定位边界。不得把多个事件合并到无法区分的一段实现中；若底层实现必须连续完成多个事件，也必须在上层保留事件函数、状态采纳、检查和 checkpoint 边界。
8. **检查先于 checkpoint**：`depends_on`、`ensures` 和 `invariant` 必须转化为可执行检查、构建期检查或测试断言。只有检查成功后才能提交目标状态、发出 checkpoint 或使后续对象依赖该事实。
9. **例外必须显式备案**：架构、链接器、Rust 语言或启动 ABI 确实要求某段代码不能放在默认映射文件时，必须在对应对象文件保留 owning wrapper 或 extern 边界，并在规格或计划中记录例外。“简单”“稳定”“方便”不是有效例外理由。
10. **验证门禁不可跳过**：涉及模型、Phase 调用链、对象状态或入口路径的修改，必须至少运行 `make build` 和 `make verify`；影响启动路径时还必须运行 `make run LOG=trace`。若验证失败，不得提交或继续扩展实现。

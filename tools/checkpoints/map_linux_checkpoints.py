...
def default_mapping_rules() -> dict[str, MappingRule]:
    return {
        ...
        "Scheduler.Schedule": MappingRule(
            mapping_kind="range",
            linux_file="kernel/sched/core.c",
            linux_symbol="__schedule",
            start_anchor_pattern=r"^\tlkm_sched_checkpoint_record\(\n\t\tLKM_SCHED_STAGE_ENTRY,",
            end_anchor_pattern=r"^\tschedule_debug\(prev, preempt\);",
            confidence="high",
            notes="Read-only differential entry interval after resolving this CPU rq->curr and before schedule_debug().",
        ),
        "Scheduler.PreparePrev.Entry": MappingRule(
            mapping_kind="range",
            linux_file="kernel/sched/core.c",
            linux_symbol="__schedule",
            start_anchor_pattern=r"^\tprev_state\s*=\s*READ_ONCE\(prev->__state\);",
            end_anchor_pattern=r"^\tlkm_sched_checkpoint_record\(\n\t\tLKM_SCHED_STAGE_PREPARE_PREV, cpu, sched_mode, prev->pid, -1,",
            confidence="high",
            notes="Linux __schedule() prev-state load through the completed sleep/signal/deactivation decision.",
        ),
        "Scheduler.PreparePrev.Exit": MappingRule(
            mapping_kind="range",
            linux_file="kernel/sched/core.c",
            linux_symbol="__schedule",
            start_anchor_pattern=r"^\tlkm_sched_checkpoint_record\(\n\t\tLKM_SCHED_STAGE_PREPARE_PREV, cpu, sched_mode, prev->pid, -1,",
            end_anchor_pattern=r"^\tnext\s*=\s*pick_next_task\(rq, prev, &rf\);",
            confidence="high",
            notes="Read-only differential PreparePrev exit through the pick_next_task() call boundary.",
        ),
        "Scheduler.DeactivateTask": MappingRule(
            mapping_kind="range",
            linux_file="kernel/sched/core.c",
            linux_symbol="__schedule",
            start_anchor_pattern=r"^\t\t\tblock_task\(rq, prev, flags\);",
            end_anchor_pattern=r"^\t\t\tblock\s*=\s*true;",
            confidence="high",
            notes="Linux non-preempt sleeping-prev removal from its runqueue before selection.",
        ),
        "Scheduler.PickNextTask.Exit": MappingRule(
            mapping_kind="range",
            linux_file="kernel/sched/core.c",
            linux_symbol="__schedule",
            start_anchor_pattern=r"^\tlkm_sched_checkpoint_record\(\n\t\tLKM_SCHED_STAGE_PICK_NEXT, cpu, sched_mode, prev->pid, next->pid,",
            end_anchor_pattern=r"^picked:",
            confidence="high",
            notes="Read-only differential interval after pick_next_task() returns prev/next/class and before picked bookkeeping.",
        ),
        "Scheduler.SwitchTo.Entry": MappingRule(
            mapping_kind="range",
            linux_file="kernel/sched/core.c",
            linux_symbol="__schedule",
            start_anchor_pattern=r"^\t\tlkm_sched_checkpoint_record\(\n\t\t\tLKM_SCHED_STAGE_SWITCH_ENTRY,",
            end_anchor_pattern=r"^\t\trq->nr_switches\+\+;",
            confidence="high",
            notes="Read-only checkpoint inside prev != next and before publishing rq->curr; identity selection cannot reach it.",
        ),
        "Scheduler.SwitchTo.Exit": MappingRule(
            mapping_kind="range",
            linux_file="kernel/sched/core.c",
            linux_symbol="finish_task_switch",
            start_anchor_pattern=r"^\tlkm_sched_checkpoint_record\(\n\t\tLKM_SCHED_STAGE_FINISH_RETURN,",
            end_anchor_pattern=r"^\treturn rq;",
            confidence="high",
            notes="Read-only next-stack checkpoint after finish-task-switch cleanup and before returning.",
        ),

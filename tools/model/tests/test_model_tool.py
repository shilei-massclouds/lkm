...
class ModelToolTests(unittest.TestCase):
    ...
    def test_model_json_contains_indexed_children_and_events(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                objects["LinuxRiscv64KernelBootSpec"]["parent"], "Kernel"
            )
            self.assertEqual(
                objects["LinuxRiscv64KernelBootSpec"]["properties"]["source"],
            ...
            self.assertIn("BootCpuRegisters.satp == 0", kernel_dependencies)
            self.assertNotIn(

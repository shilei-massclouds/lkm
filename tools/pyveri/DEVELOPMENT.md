...
- `BootArgs.Online` 的三条入口启动参数事实已经由窄规则收口：`attrs_accessible(self)`、`boot_hartid == Riscv64.a0`、`dtb_pa == Riscv64.a1` 均归入 `boot_arguments / riscv_boot_protocol` 并直接证明。依据是本地 Linux `../linux-6.12/Documentation/arch/riscv/boot.rst` 的 RISC-V boot requirements：入口 `$a0` 保存当前 hartid，`$a1` 保存内存中 devicetree 地址。该规则只作用于 `BootArgs.State::Online`，不把其它对象的 `attrs_accessible(self)` 泛化为自动证明。

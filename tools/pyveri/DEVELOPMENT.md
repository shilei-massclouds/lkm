...
- `BootArgs` 是 `FirmwareProject.Setup` 构造的只读 boot ABI 产物；`attrs_accessible(self)` 归入 `firmware_boot_abi / firmware_project_boot_abi`。`OpenSBI.Enable` 只保证 `BootCpuRegisters.a0/a1` 与 `BootArgs.boot_hartid/dtb_pa` 一致，不能用入口寄存器反向定义这个静态产物，也不能据此合成其它寄存器值。

...
- `CpuGroup.Preset` 已改为依赖 `BootArgs.boot_hartid`，并用 `ensures { boot_cpu_hartid == BootArgs.boot_hartid; }` 表达事件完成后记录启动 hart 标识。这样 `BootArgs` 成为后续对象使用启动参数的统一抽象；`valid_hart_id(BootArgs.boot_hartid)` 和 `valid_hart_id(boot_cpu_hartid)` 仍保留为 FDT/平台 CPU 描述共同证明的义务。

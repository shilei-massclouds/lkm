...
- `valid_hart_id(...)` 已被更具体的 `platform_hart_id_valid(...)` 取代。当前已引入 `PlatformCpuInfo` 准备期对象，表示从 FDT/platform CPU 描述中提取出的启动处理器信息；`PlatformCpuInfo.boot_hartid == BootArgs.boot_hartid` 和 `platform_hart_id_valid(PlatformCpuInfo.boot_hartid)` 归入 `platform_cpu_description / fdt_cpu_description`。`CpuGroup` 中的 `platform_hart_id_valid(BootArgs.boot_hartid)` 与 `platform_hart_id_valid(boot_cpu_hartid)` 由这些前序事实推出。

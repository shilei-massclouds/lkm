...
- `interrupt_concurrency_closed()` 的来源是 RISC-V 入口代码和入口前导期的中断流状态。已核对本地 Linux 源码 `~/gitStudy/linux-6.12.37/arch/riscv/kernel/head.S`：`_start_kernel` 入口直接执行 `csrw CSR_IE, zero` 与 `csrw CSR_IP, zero`，关闭中断使能与挂起状态；当前 `.spec` 中 `InterruptStream.Preset` 也用 `ensures` 建立 `Riscv64.sie == 0` 和 `Riscv64.sip == 0`，`InterruptStream.Prepared` 保持这两条 invariant。因此该事实归入 `system_exclusive_context / riscv_entry_and_interrupt_stream`。

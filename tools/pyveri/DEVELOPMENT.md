...
- 当前以 `ComputerProject.Transition::Preset` 从 `State::Base` 到 `State::Prepared` 的推导作为结构化 trace 入口；该入口顺序驱动 `HardwareProject`、`FirmwareProject`、`KernelProject`，完成构造后交接给系统根 `Computer`，再异步推进 `Riscv64Platform -> OpenSBI -> Kernel`。
...
- `BootArgs` 是 `FirmwareProject.Setup` 构造的只读 boot ABI 产物；`attrs_accessible(self)` 归入 `firmware_boot_abi / firmware_project_boot_abi`。`OpenSBI.Enable` 再保证 `BootHartContext.a0/a1` 与 `BootArgs.boot_hartid/dtb_pa` 一致，不能用入口寄存器反向定义这个静态产物。
...
- `Riscv64.attrs_accessible(self)` 归入 `architecture_capabilities / riscv_isa_spec`，只表示外部 ISA 能力可用；可变 GPR/CSR 不再是 `Riscv64` 属性，而由 `BootHartContext` 保存和推进。
- `EntryPreludePhase.Preset` 已用 transition 后置条件证明 `kernel_fpu_disabled(BootHartContext.sstatus)` 和 `kernel_vector_disabled(BootHartContext.sstatus)`。依据是 RISC-V ISA 定义 `sstatus` 中的 `SR_FS/SR_VS` 状态位，Linux `arch/riscv/kernel/head.S` 入口路径清除这些位。

后续规格结构清理：

- 已引入 `BootArgs` 对象作为 FirmwareProject 定义的内核启动 ABI；`OpenSBI.Enable` 把它物化到 `BootHartContext.a0/a1`。`RawDtb` 依赖 `BootArgs.dtb_pa`，`CpuGroup` 依赖 `BootArgs.boot_hartid`，避免裸寄存器名散落在规格中。
...
- 推导器已开始记录已证明表达式，并用前序事实继续证明一小组派生 invariant：`valid_task_ref(BootHartContext.tp)`、`valid_stack_pointer(BootHartContext.sp)` 和 `inside(BootHartContext.sp, ...)` 可由已证明的 `tp/sp` 后置关系推出。
...
- 预览输出文件不要散落在仓库根目录。`-T/--trace` 不带路径时已默认输出到 `tools/out/trace/`；显式路径仍用于正式导出到 `spec/charter/pic/` 或其它指定目录。
